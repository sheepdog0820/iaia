...
from .models import PlayHistory, Scenario, ScenarioHandout, ScenarioHandoutRecommendedSkill, ScenarioRecommendedSkill
...
class ScenarioModelsTestCase(TestCase):
    ...
    def test_play_history_creation(self):
        ...
        play_history = PlayHistory.objects.create(
            scenario=scenario, user=self.user, played_date=timezone.now(), role="gm", notes="Test session notes"
        )
    ...
    def test_play_history_string_representation(self):
        ...
        play_history = PlayHistory.objects.create(
            scenario=scenario, user=self.user, played_date=timezone.now(), role="gm"
        )
...
class ScenarioAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.play_history = PlayHistory.objects.create(
            scenario=self.scenario, user=self.user1, played_date=timezone.now(), role="gm", notes="Test notes"
        )
    ...
    def test_scenario_recommended_skills_blank(self):
        ...
        patch_response = self.client.patch(f"/api/scenarios/scenarios/{scenario_id}/", {"recommended_skills": "   "})
