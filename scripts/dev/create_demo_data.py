...
def create_demo_data():
    ...
    try:
        ...
        demo_user = User.objects.create_user(
            username="demo_user", email="demo@example.com", password="demo123", nickname="デモユーザー"
        )
        print("デモユーザーを作成しました")
    ...
    try:
        ...
        gm_user = User.objects.create_user(
            username="demo_gm", email="gm@example.com", password="gm123", nickname="デモGM"
        )
        print("デモGMを作成しました")
    ...
    for scenario_data in scenarios_data:
        scenario, created = Scenario.objects.get_or_create(title=scenario_data["title"], defaults=scenario_data)
    ...
    for session_data in sessions_data:
        session, created = TRPGSession.objects.get_or_create(
            title=session_data["title"], gm=session_data["gm"], defaults=session_data
        )
    ...
    for session in created_sessions:
        participant, created = SessionParticipant.objects.get_or_create(
            session=session,
            user=demo_user,
            defaults={"role": "player", "character_name": f"探索者{demo_user.nickname}"},
        )
        if created:
