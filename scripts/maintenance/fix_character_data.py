...
def fix_character_data():
    ...
    characters_to_fix = (
        CharacterSheet.objects.filter(hit_points_max__isnull=True)
        | CharacterSheet.objects.filter(magic_points_max__isnull=True)
        | CharacterSheet.objects.filter(sanity_max__isnull=True)
    )
    ...
    for character in characters_to_fix:
        ...
        if character.edition == "6th":
            ...
            if character.sanity_max is None:
                ...
                cthulhu_skill = character.skills.filter(skill_name__icontains="クトゥルフ").first()
    ...
    # Verify fix
    remaining = CharacterSheet.objects.filter(hit_points_max__isnull=True).count()
