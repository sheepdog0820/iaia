...
def migrate_character_images():
    ...
    characters_with_old_images = CharacterSheet.objects.filter(character_image__isnull=False).exclude(
        character_image=""
    )
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    with character.character_image.open("rb") as f:
                        ...
                        new_image = CharacterImage(character_sheet=character, is_main=True, order=0)
...
def cleanup_old_images(dry_run=True):
    ...
    characters_to_clean = CharacterSheet.objects.filter(images__isnull=False, character_image__isnull=False).distinct()
