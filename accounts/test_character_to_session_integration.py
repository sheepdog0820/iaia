...
class CharacterToSessionIntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        GroupMembership.objects.create(user=self.player3, group=self.group, role="member")
    ...
    def test_complete_flow_from_character_creation_to_session(self):
        ...
        # セッション参加履歴の確認
        self.assertTrue(
            SessionParticipant.objects.filter(session=session, user=self.player1, character_sheet=char1).exists()
        )
