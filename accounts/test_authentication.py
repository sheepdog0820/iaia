...
class GroupTestCase(TestCase):
    ...
    def test_group_creation(self):
        """グループ作成のテスト"""
        group = CustomGroup.objects.create(name="Test Group", description="Test Description", created_by=self.user1)
        self.assertEqual(group.name, "Test Group")
        self.assertEqual(group.created_by, self.user1)

    def test_group_membership(self):
        """グループメンバーシップのテスト"""
        group = CustomGroup.objects.create(name="Test Group", created_by=self.user1)
