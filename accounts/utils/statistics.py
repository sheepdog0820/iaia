...
class SessionStatistics:
    ...
    def calculate_session_stats(sessions_queryset, user=None):
        ...
        stats = {
            ...
            "total_minutes": sessions_queryset.aggregate(total=Sum(effective_duration_expression()))["total"] or 0,
        }
    ...
    def calculate_yearly_stats(sessions_queryset, year=None, user=None):
        ...
        yearly_sessions = sessions_queryset.filter(date__year=year, status="completed")
    ...
    def calculate_monthly_breakdown(sessions_queryset, year=None):
        ...
        for month in range(1, 13):
            month_sessions = sessions_queryset.filter(date__year=year, date__month=month, status="completed")
    ...
    def calculate_role_distribution(sessions_queryset, user):
        ...
        return {
            "gm_stats": gm_stats,
            "pl_stats": pl_stats,
            "total_unique_sessions": sessions_queryset.filter(Q(gm=user) | Q(participants=user)).distinct().count(),
        }
...
class CharacterStatistics:
    ...
    def calculate_character_stats(characters_queryset):
        ...
        # エディション別分布
        edition_breakdown = characters_queryset.values("edition").annotate(count=Count("id")).order_by("edition")

        stats["edition_breakdown"] = {item["edition"]: item["count"] for item in edition_breakdown}
    ...
    def calculate_skill_statistics(skills_queryset):
        ...
        stats = {
            ...
            "average_skill_value": skills_queryset.aggregate(avg=Avg("current_value"))["avg"] or 0,
        }

        # カテゴリ別分布
        category_breakdown = (
            skills_queryset.values("category")
            .annotate(count=Count("id"), avg_value=Avg("current_value"))
            .order_by("category")
        )

        stats["category_breakdown"] = {
            item["category"]: {"count": item["count"], "average_value": round(item["avg_value"] or 0, 1)}
    ...
    def calculate_group_stats(groups_queryset, user=None):
        ...
        if user:
            ...
            stats["user_admin_groups"] = user_groups.filter(
                groupmembership__user=user, groupmembership__role="admin"
            ).count()

        # メンバー数統計
        member_stats = groups_queryset.aggregate(total_members=Sum("members__count"), avg_members=Avg("members__count"))
...
class ExportStatistics:
    ...
    def prepare_export_data(user, year=None):
        ...
        # セッション統計
        user_sessions = TRPGSession.objects.filter(Q(gm=user) | Q(participants=user)).distinct()
        ...
        return {
            ...
            "summary": {
                ...
                "latest_session_date": (
                    latest_session.date.isoformat() if latest_session and latest_session.date else None
                ),
            },
        }
...
class TindalosMetrics:
    ...
    def calculate_tindalos_format(user, year=None):
        ...
        user_sessions = TRPGSession.objects.filter(
            Q(gm=user) | Q(participants=user), date__year=year, status="completed"
        ).distinct()
        ...
        # 効率性メトリクス
        total_hours = (user_sessions.aggregate(total=Sum(effective_duration_expression()))["total"] or 0) / 60
        ...
        return {
            ...
            "balance_score": TindalosMetrics._calculate_balance_score(gm_sessions.count(), pl_sessions.count()),
    ...
    def _calculate_consistency_score(sessions_queryset, year):
        ...
        for month in range(1, 13):
            month_count = sessions_queryset.filter(date__year=year, date__month=month).count()
