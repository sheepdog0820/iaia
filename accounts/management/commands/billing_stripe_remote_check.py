    ...
    def handle(self, *args, **options):
        ...
        if yearly_price_id:
            ...
            if yearly_price_errors:
                raise CommandError(
                    'Stripe yearly price check failed: ' + ', '.join(yearly_price_errors)
                )
        ...
        if options['require_recent_events']:
            ...
            if recent_event_errors:
                raise CommandError(
                    'Stripe recent event check failed: ' + ', '.join(recent_event_errors)
                )
            for line in summarize_recent_operational_events(recent_events):
                self.stdout.write(line)
            self.stdout.write(
                self.style.SUCCESS(
                    f'OK stripe recent operational events last {options["recent_hours"]}h'
                )
            )
        self.stdout.write(self.style.SUCCESS('billing_stripe_remote_check=ok'))
...
def validate_portal_configurations(
    ...
    if expected_configuration_id:
        ...
        active_configurations = [
            ...
            if _get(configuration, 'active', False)
        ]
    ...
    known_mode_configurations = [
        ...
        if _get(configuration, 'livemode') is not None
    ]
    if expected_livemode is not None and known_mode_configurations:
        ...
        if not matching_configurations:
            expected = 'live' if expected_livemode else 'test'
            actual_modes = {
                'live' if _get(configuration, 'livemode') else 'test'
                for configuration in known_mode_configurations
            }
            return [
                'customer portal livemode mismatch: '
                f'expected {expected}, got {", ".join(sorted(actual_modes))}'
            ]
    feature_errors = []
    ...
    for feature_key, label in feature_requirements.items():
        if not any(
            portal_feature_enabled(configuration, feature_key)
            for configuration in active_configurations
        ):
...
def validate_recent_operational_events(events, expected_livemode=None):
    ...
    if expected_livemode is not None:
        mismatched = [
            ...
            for event in data
            if _get(event, 'livemode') is not None
            and _get(event, 'livemode') != expected_livemode
        ]
        if mismatched:
            expected = 'live' if expected_livemode else 'test'
            errors.append(
                f'recent event livemode mismatch: expected {expected}, events {", ".join(mismatched)}'
            )
    ...
    canceling_subscription_events = [
        ...
        and _get(_get(_get(event, 'data', {}) or {}, 'object', {}) or {}, 'cancel_at_period_end')
        is True
    ]
...
def summarize_recent_operational_events(events):
    ...
    lines.append(
        ...
        + (', '.join(canceling_event_ids) if canceling_event_ids else '-')
    )
    return lines
