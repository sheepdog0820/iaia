...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self._stripe_key_check(
            values, "STRIPE_SECRET_KEY", "sk_test_", "sk_live_", options["require_stripe"], warnings, errors
        )
        self._stripe_key_check(
            values, "STRIPE_PUBLISHABLE_KEY", "pk_test_", "pk_live_", options["require_stripe"], warnings, errors
        )
        self._stripe_key_check(
            values, "STRIPE_WEBHOOK_SECRET", "whsec_", None, options["require_stripe"], warnings, errors
        )
        self._stripe_key_check(
            values, "STRIPE_PREMIUM_PRICE_ID", "price_", None, options["require_stripe"], warnings, errors
        )
        self._stripe_key_check(
            values, "STRIPE_PREMIUM_YEARLY_PRICE_ID", "price_", None, options["require_stripe"], warnings, errors
        )
