...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f'refund_or_dispute_active_records={report["refund_or_dispute_active_records"]}'
        )
        ...
        self.stdout.write(
            f'stale_processing_webhook_events={report["stale_processing_webhook_events"]}'
        )
        self.stdout.write(f'last_failed_webhook_event_id={report["last_failed_webhook_event_id"] or "(none)"}')
        self.stdout.write(
            f'last_failed_webhook_event_type={report["last_failed_webhook_event_type"] or "(none)"}'
        )
        self.stdout.write(
            f'last_stale_processing_webhook_event_id='
            f'{report["last_stale_processing_webhook_event_id"] or "(none)"}'
        )
...
def build_billing_status_report(now=None):
    ...
    statuses = {
        ...
        for row in records.values('subscription_status')
    ...
    manual_override_without_subscription_ids = (
        manual_override_user_ids - subscription_user_ids
    )
    ...
    billing_intervals = {
        ...
        for row in records.values('billing_interval')
        .annotate(count=Count('id'))
    ...
    last_stale_processing_webhook_event = (
        stale_processing_webhook_events.order_by('-processed_at').first()
    )
    ...
    return {
        ...
        'refund_or_dispute_active_records': sum(
            ...
            for record in records
            if (
                record.last_refund_or_dispute_at is not None
                and record.expected_user_premium_access
            )
        ),
        ...
        'last_webhook_processed_at': (
            last_webhook_event.processed_at.isoformat() if last_webhook_event else ''
        ),
        ...
        'last_failed_webhook_event_id': (
            last_failed_webhook_event.event_id if last_failed_webhook_event else ''
        ),
        'last_failed_webhook_event_type': (
            last_failed_webhook_event.event_type if last_failed_webhook_event else ''
        ),
        'last_stale_processing_webhook_event_id': (
            last_stale_processing_webhook_event.event_id
            if last_stale_processing_webhook_event
            else ''
        ),
        'statuses': statuses,
    }
