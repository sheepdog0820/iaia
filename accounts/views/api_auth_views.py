...
def logout(request):
    ...
    try:
        ...
        return Response(
            {"error": "ログアウト処理中にエラーが発生しました"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def twitter_auth(request):
    ...
    try:
        ...
        if not access_token:
            return Response({"error": "アクセストークンが取得できませんでした。"}, status=status.HTTP_400_BAD_REQUEST)
...
def discord_auth(request):
    ...
    if not code and not access_token:
        return Response({"error": "code または access_token が必要です。"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        if not access_token:
            ...
            if code_verifier:
                ...
                return Response(
                    {"error": "client_secret または code_verifier が必要です。"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if not access_token:
                return Response(
                    {"error": "アクセストークンが取得できませんでした。"}, status=status.HTTP_400_BAD_REQUEST
                )
        ...
        if not discord_id:
            return Response(
                {"error": "DiscordのユーザーIDが取得できませんでした。"}, status=status.HTTP_400_BAD_REQUEST
            )
