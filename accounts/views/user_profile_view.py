...
class UserProfileView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        shared_group_ids = list(GroupMembership.objects.filter(user=viewer).values_list("group_id", flat=True))
        shared_groups = list(
            GroupMembership.objects.filter(user=profile_user, group_id__in=shared_group_ids).select_related("group")
        )
            ...
            sections_raw = [
                ...
                (
                    ...
                    [
                        ...
                        (
                            "物語・展開の嗜好",
                            [preference_labels.get(v, v) for v in (scenario.get("preferences") or [])],
                        ),
                ...
                (
                    ...
                    [
                        (
                            "ロールプレイスタイル",
                            [rp_style_labels.get(v, v) for v in (character_play.get("rp_style") or [])],
                        ),
                        (
                            "得意・好きな役割",
                            [role_pref_labels.get(v, v) for v in (character_play.get("role_preference") or [])],
                        ),
                    ],
                ),
                (
