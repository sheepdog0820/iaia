...
class CharacterSheetAccessMixin:
    ...
    def can_read_character_sheet(character_sheet, user):
        ...
        if access_scope == "group":
            ...
            owner_group_ids = GroupMembership.objects.filter(user_id=character_sheet.user_id).values_list(
                "group_id", flat=True
            )
...
class ErrorHandlerMixin:
    ...
    def handle_not_found(resource_name="リソース"):
        """404エラーレスポンス生成"""
        return Response({"error": f"{resource_name}が見つかりません"}, status=status.HTTP_404_NOT_FOUND)

    @staticmethod
    def handle_already_exists(resource_name="リソース"):
        """400エラーレスポンス生成（既存エラー）"""
        return Response({"error": f"{resource_name}は既に存在します"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def validate_bulk_data(self, data_list, required_fields=None):
        ...
        if required_fields:
            for i, data in enumerate(data_list):
                ...
                if missing_fields:
                    raise ValidationError(f"アイテム{i+1}: 必須フィールドが不足しています: {', '.join(missing_fields)}")
