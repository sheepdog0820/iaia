...
def _ensure_social_apps():
    providers = set(
        SocialApp.objects.filter(provider__in=["google", "twitter_oauth2", "discord"]).values_list(
            "provider", flat=True
        )
    )
    ...
    if not _is_placeholder(google_client_id, google_placeholders) and not _is_placeholder(
        google_secret, google_placeholders
    ):
        google_app, created = SocialApp.objects.get_or_create(
            provider="google", defaults={"name": "Google", "client_id": google_client_id, "secret": google_secret}
        )
        if not created:
            updated = False
    ...
    if not _is_placeholder(discord_client_id, discord_placeholders) and not _is_placeholder(
        discord_secret, discord_placeholders
    ):
        discord_app, created = SocialApp.objects.get_or_create(
            provider="discord", defaults={"name": "Discord", "client_id": discord_client_id, "secret": discord_secret}
        )
        if not created:
            updated = False
    ...
    if not _is_placeholder(twitter_client_id, twitter_placeholders) and not _is_placeholder(
        twitter_secret, twitter_placeholders
    ):
        twitter_app, created = SocialApp.objects.get_or_create(
            provider="twitter_oauth2", defaults={"name": "X", "client_id": twitter_client_id, "secret": twitter_secret}
        )
        if not created:
            updated = False
...
class DashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        sessions_qs = TRPGSession.objects.filter(Q(participants=self.request.user) | Q(gm=self.request.user)).distinct()
        ...
        context["upcoming_sessions"] = (
            TRPGSession.objects.filter(
                Q(participants=self.request.user) | Q(gm=self.request.user), date__gte=timezone.now()
            )
            .distinct()
            .order_by("date")[:5]
        )
        ...
        context["recent_sessions"] = (
            TRPGSession.objects.filter(
                Q(participants=self.request.user) | Q(gm=self.request.user), date__lt=timezone.now()
            )
            .distinct()
            .order_by("-date")[:5]
        )
...
def demo_login_page(request):
    ...
    return render(request, "account/demo_login.html", {"providers": providers, "demo_mode": True})
...
def mock_social_login(request, provider):
    ...
    social_account, created = SocialAccount.objects.get_or_create(
        user=user, provider=provider, defaults={"uid": f"{provider}_demo_uid"}
    )
