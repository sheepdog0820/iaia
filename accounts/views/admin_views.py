...
class AdminUserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .prefetch_related(
                "created_groups", "groupmembership_set", "character_sheets", "gm_sessions", "sessionparticipant_set"
            )
            .annotate(
    ...
    def set_staff(self, request, pk=None):
        ...
        user.save()
        return Response(
            {"message": f"{user.username}をスタッフに設定しました", "user": UserDetailSerializer(user).data}
        )
    ...
    def unset_staff(self, request, pk=None):
        ...
        if user.is_superuser:
            return Response(
                {"error": "スーパーユーザーのスタッフ権限は解除できません"}, status=status.HTTP_400_BAD_REQUEST
            )
        user.is_staff = False
        user.save()
        return Response(
            {"message": f"{user.username}のスタッフ権限を解除しました", "user": UserDetailSerializer(user).data}
        )
    ...
    def activate(self, request, pk=None):
        ...
        user.is_active = True
        user.save()
        return Response({"message": f"{user.username}をアクティブ化しました", "user": UserDetailSerializer(user).data})
    ...
    def deactivate(self, request, pk=None):
        ...
        user = self.get_object()
        if user.is_superuser:
            return Response({"error": "スーパーユーザーは非アクティブ化できません"}, status=status.HTTP_400_BAD_REQUEST)
        user.is_active = False
        user.save()
        return Response(
            {"message": f"{user.username}を非アクティブ化しました", "user": UserDetailSerializer(user).data}
        )
        ...
        return Response(
            {
                ...
                "gm_sessions": [
                    {"id": s.id, "title": s.title, "date": s.date, "participant_count": s.participants.count()}
