...
class Character6thCalculationTestCase(TestCase):
    ...
    def test_damage_bonus_calculation(self):
        ...
        for str_val, siz_val, expected_db in test_cases:
            character = create_test_character(
                self.user, name=f"DB Test STR={str_val} SIZ={siz_val}", str_value=str_val, siz_value=siz_val
            )
    ...
    def test_occupation_points_calculation(self):
        ...
        character = create_test_character(self.user, name="Occupation Points Test", edu_value=18)
    ...
    def test_hobby_points_calculation(self):
        ...
        for int_val in test_cases:
            character = create_test_character(self.user, name=f"Hobby Test INT={int_val}", int_value=int_val)
...
class Character6thAPITestCase(APITestCase):
    ...
    def test_create_6th_edition_character_with_scenario_linkage(self):
        ...
        scenario = Scenario.objects.create(title="リンクシナリオ", game_system="coc", created_by=self.user)
        data = {
    ...
    def test_public_character_access(self):
        ...
        private_char = create_test_character(self.other_user, name="Private Character", access_scope="private")
        public_char = create_test_character(self.other_user, name="Public Character", access_scope="public")
    ...
    def test_skill_points_allocation_api(self):
        ...
        response = self.client.post(
            ...
            {"skill_id": skill.id, "occupation_points": 70, "interest_points": 20},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class Character6thIntegrationTestCase(TransactionTestCase):
    ...
    def test_complete_character_creation_workflow(self):
        ...
        for skill in skills:
            response = self.client.post(f"/api/accounts/character-sheets/{character_id}/skills/", skill, format="json")
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        equipment = [
            {"item_type": "weapon", "name": "Pistol", "damage": "1D10", "attacks_per_round": 2, "weight": "0.5"},
            {"item_type": "armor", "name": "Kevlar Vest", "armor_points": 8, "weight": "3.0"},
            {"item_type": "item", "name": "Medical Kit", "weight": "2.0", "quantity": 1},
        ]

        for item in equipment:
            response = self.client.post(
                f"/api/accounts/character-sheets/{character_id}/equipment/", item, format="json"
            )
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.client.post(
            f"/api/accounts/character-sheets/{character_id}/background/", background_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class Character6thErrorHandlingTestCase(APITestCase):
    ...
    def test_skill_overallocation(self):
        ...
        with self.assertLogs("django.request", level="WARNING"):
            response = self.client.post(
                f"/api/accounts/character-sheets/{character.id}/allocate_skill_points/",
                {"skill_id": skill.id, "occupation_points": 999, "interest_points": 0},
                format="json",
            )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_large_data_handling(self):
        ...
        allocations = [{"skill_id": skill.id, "occupation_points": 1, "interest_points": 0} for skill in skills]
    ...
    def test_file_upload_validation(self):
        ...
        with patch("django.core.files.uploadedfile.SimpleUploadedFile") as mock_file:
            ...
            with self.assertLogs("django.request", level="WARNING"):
                response = self.client.post(
                    f"/api/accounts/character-sheets/{character.id}/images/", {"image": mock_file.return_value}
                )
