...
class ShareLinkApiTests(APITestCase):
    ...
    def test_issue_share_link_for_link_session_and_shared_api_hides_secret_data(self):
        character = self.create_character(access_scope='public', is_public=True)
    ...
    def test_shared_session_uses_legacy_gm_identity_without_internal_owner_name(self):
        session = self.create_session()
        identity = ParticipantIdentity.objects.create(
            display_name='Legacy GM Display',
            legacy_source='csv',
            legacy_key='legacy-gm',
        )
        ...
        self.assertEqual(shared_response.json()['gm_name'], 'Legacy GM Display')
    ...
    def test_share_stats_token_returns_aggregate_without_user_identity_linking(self):
        ...
        self.assertEqual(data['participants'][0]['display_name'], 'Legacy GM')
    ...
    def test_participant_identity_alias_is_name_based_and_not_user_linked(self):
        identity = ParticipantIdentity.objects.create(
            display_name='Kenta',
            legacy_source='spreadsheet',
            legacy_key='legacy-kenta',
        )
        alias = identity.aliases.create(alias='Endo', source='spreadsheet')
