...
def active_subscription_query(now=None):
    ...
    ) & (models.Q(premium_expires_at__isnull=True) | models.Q(premium_expires_at__gt=now))
...
def manual_override_user_ids():
    latest_manual_logs = {}
    logs = (
        PremiumAuditLog.objects.filter(source="manual", action__in=("granted", "revoked"))
...
class PremiumAccessCodeAdmin(admin.ModelAdmin):
    ...
    def export_codes_csv(self, request, queryset):
        ...
        for access_code in queryset.select_related("created_by").prefetch_related("redemptions__user"):
            ...
            redemption_usernames = [redemption.user.username for redemption in redemptions]
            redemption_emails = [redemption.user.email for redemption in redemptions if redemption.user.email]
            redemption_details = [
...
class PremiumAuditLogAdmin(admin.ModelAdmin):
    ...
    def metadata_summary(self, obj):
        if not obj.metadata:
            return "-"
        parts = [f"{key}={value}" for key, value in sorted(obj.metadata.items())]
        summary = ", ".join(parts)
