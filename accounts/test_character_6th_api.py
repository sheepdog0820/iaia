...
class Character6thAPITestCase(APITestCase):
    ...
    def test_ccfolia_import_api_endpoint(self):
        ...
        response = self.client.post(url, {"ccfolia": ccfolia_payload, "age": 20}, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_ccfolia_import_api_endpoint_accepts_7th_edition(self):
        ...
        response = self.client.post(url, {"ccfolia": ccfolia_payload, "age": 22}, format="json")
...
class CCFOLIASynchronizationTestCase(TestCase):
    ...
    def test_sync_conflict_resolution(self):
        """同期競合解決のテスト"""
        # 同期競合が発生した場合の処理
        conflict_data = {"local_version": 2, "remote_version": 3, "conflict_fields": ["str_value", "skills"]}
...
class BulkExportTestCase(TestCase):
    ...
    def test_export_format_validation(self):
        ...
        for character in self.characters:
            ...
            # 必須フィールドの存在確認
            required_fields = ["name", "initiative", "commands", "params", "status"]
