...
class PremiumSubscription(models.Model):
    ...
    def is_stripe_active(self):
        return (
            self.subscription_status in self.ACTIVE_STATUSES
    ...
    def is_promo_active(self):
        return (
            ...
            and self.revoked_at is None
            and (
                self.premium_expires_at is None
                or self.premium_expires_at > timezone.now()
            )
        )
    ...
    def revoke_access(self, reason, save=True, preserve_manual_override=False):
        ...
        self.user.is_premium = (
            self.has_manual_premium_override
