...
class SimpleTindalosMetricsView(APIView):
    ...
    def get(self, request):
        ...
        scenario_count = PlayHistory.objects.filter(play_history_filter).values("scenario").distinct().count()
    ...
    def _get_yearly_trends(self, user):
        ...
        first_session = TRPGSession.objects.filter(Q(participants=user) | Q(gm=user)).order_by("date").first()
    ...
    def _get_monthly_details(self, user, year):
        ...
        for month in range(1, 13):
            ...
            # 月別シナリオランキング（上位5件）
            scenarios = (
                PlayHistory.objects.filter(user=user, played_date__year=year, played_date__month=month)
                .values("scenario__id", "scenario__title", "scenario__game_system")
    ...
    def _get_popular_scenarios(self, user, year=None):
        ...
        popular_scenarios = (
            PlayHistory.objects.filter(filters)
            .values("scenario__id", "scenario__title", "scenario__game_system", "scenario__difficulty")
            .annotate(play_count=Count("id"), last_played=Max("played_date"))
    ...
    def _get_system_trends(self, user):
        ...
        for year in range(start_year, current_year + 1):
            system_stats = (
                PlayHistory.objects.filter(user=user, played_date__year=year)
                .values("scenario__game_system")
            ...
            for stat in system_stats:
                ...
                if system_name not in trends:
                    trends[system_name] = {"system_code": system, "system_name": system_name, "data": []}

                trends[system_name]["data"].append({"year": year, "count": stat["count"]})
...
class TindalosMetricsView(APIView):
    ...
    def _get_weekly_stats(self, user, year):
        ...
        sessions = TRPGSession.objects.filter(
            Q(participants=user) | Q(gm=user), date__year=year, status="completed"
        ).distinct()
    ...
    def _get_yearly_stats(self, user, year):
        ...
        # 参加グループ数
        active_groups = (
            Group.objects.filter(sessions__participants=user, sessions__date__year=year, sessions__status="completed")
            .distinct()
            .count()
        )
        ...
        # プレイしたシナリオ数
        played_scenarios = (
            PlayHistory.objects.filter(user=user, played_date__year=year).values("scenario").distinct().count()
        )
    ...
    def _get_monthly_stats(self, user, year):
        ...
        for month in range(1, 13):
            sessions = TRPGSession.objects.filter(
                (Q(participants=user) | Q(gm=user)), date__year=year, date__month=month, status="completed"
            ).distinct()
    ...
    def _get_role_stats(self, user, year):
        """役割別統計（GM/PL）"""
        # GM統計
        gm_sessions = TRPGSession.objects.filter(gm=user, date__year=year, status="completed")
        ...
        # PL統計
        pl_sessions = TRPGSession.objects.filter(participants=user, date__year=year, status="completed").exclude(
            gm=user
        )
    ...
    def _get_group_stats(self, user, year):
        ...
        for group in user_groups:
            sessions = (
                TRPGSession.objects.filter(group=group, date__year=year, status="completed")
                .filter(Q(participants=user) | Q(gm=user))
                .distinct()
            )
            ...
            top_gm = (
                sessions.values("gm__nickname", "gm__username")
                .annotate(gm_count=Count("id"))
                .order_by("-gm_count")
                .first()
            )
    ...
    def _get_recent_sessions(self, user, limit=10):
        """最近のセッション履歴"""
        sessions = (
            TRPGSession.objects.filter(status="completed")
            .filter(Q(participants=user) | Q(gm=user))
            .distinct()
            .select_related("group", "gm")
            .order_by("-date")[:limit]
        )
    ...
    def _calculate_completion_rate(self, user, year):
        """セッション完了率の計算"""
        total_planned = (
            TRPGSession.objects.filter(Q(participants=user) | Q(gm=user), date__year=year).distinct().count()
        )

        completed = (
            TRPGSession.objects.filter(Q(participants=user) | Q(gm=user), date__year=year, status="completed")
            .distinct()
            .count()
        )
    ...
    def _calculate_current_streak(self, user):
        ...
        sessions = (
            TRPGSession.objects.filter(date__gte=eight_weeks_ago, status="completed")
            .filter(Q(participants=user) | Q(gm=user))
            .distinct()
            .order_by("-date")
        )
        ...
        for week_offset in range(8):
            ...
            week_sessions = sessions.filter(date__week=check_week)
    ...
    def _calculate_longest_streak(self, user, year):
        """年間最長連続プレイ週数"""
        sessions = (
            TRPGSession.objects.filter(date__year=year, status="completed")
            .filter(Q(participants=user) | Q(gm=user))
            .distinct()
            .order_by("date")
        )
...
class UserRankingView(APIView):
    ...
    def get(self, request):
        ...
        category_items = self._parse_int(
            request.query_params.get("category_items"), default=10, min_value=1, max_value=50
        )
        trend_periods = self._parse_int(
            request.query_params.get("trend_periods"), default=None, min_value=1, max_value=120
        )
        ...
        filtered_sessions = self._filter_sessions_by_scenario(
            base_sessions, game_system=game_system, scenario_id=scenario_id
        )
    ...
    def _build_category_rankings(
        ...
        if category == "system":
            systems = (
                [(game_system, dict(Scenario.GAME_SYSTEM_CHOICES).get(game_system, game_system))]
                if game_system
                else Scenario.GAME_SYSTEM_CHOICES
            )
            result = []
        ...
        if category == "scenario":
            ...
            for scenario in scenarios:
                sessions = self._filter_sessions_by_scenario(
                    base_sessions, scenario_id=scenario["id"], game_system=None
                )
                ranking, _ = self._build_ranking(sessions, ranking_type, limit=limit)
                if ranking:
                    result.append(
                        {
                            "scenario": {
                                ...
                                "game_system_display": dict(Scenario.GAME_SYSTEM_CHOICES).get(
                                    scenario["game_system"], scenario["game_system"]
                                ),
    ...
    def _get_hours_ranking(self, year):
        """プレイ時間ランキング"""
        users = (
            CustomUser.objects.filter(sessions__date__year=year, sessions__status="completed")
            .annotate(
    ...
    def _get_gm_ranking(self, year):
        """GMセッション数ランキング"""
        users = (
            CustomUser.objects.filter(gm_sessions__date__year=year, gm_sessions__status="completed")
            .annotate(
...
class GroupStatisticsView(APIView):
    ...
    def get(self, request):
        ...
        for stats in group_stats:
            ...
            group_info = {
                "group": {"id": stats["group_id"], "name": stats["group_name"]},
        ...
        return Response({"year": year, "groups": groups_data})

    def _get_group_stats(self, group, year):
        """個別グループの統計"""
        sessions = TRPGSession.objects.filter(group=group, date__year=year, status="completed")
        ...
        # 最も活発なGM
        top_gm = (
            sessions.values("gm__nickname", "gm__username").annotate(gm_count=Count("id")).order_by("-gm_count").first()
        )
    ...
    def _get_member_contributions(self, group, sessions, session_count=0):
        ...
        gm_stats = sessions.values("gm_id").annotate(
            session_count=Count("id"), total_minutes=Sum(effective_duration_expression())
        )
        ...
        users = CustomUser.objects.filter(id__in=contributions.keys()).values("id", "nickname", "username")
        name_map = {u["id"]: (u["nickname"] or u["username"]) for u in users}
        ...
        for user_id, data in contributions.items():
            ...
            data["participation_rate"] = (
                round((data["total_sessions"] / session_count) * 100, 1) if session_count > 0 else 0
            )
            result.append(data)
...
class DetailedTindalosMetricsView(APIView):
    ...
    def _get_summary_stats(self, user, year=None):
        ...
        # ゲームシステム別サマリー
        system_stats = play_history.values("scenario__game_system").annotate(count=Count("id")).order_by("-count")
    ...
    def _get_yearly_trends_detailed(self, user, start_year=None, end_year=None):
        ...
        return {"period": f"{start_year} - {end_year}", "years": yearly_data, "total_years": end_year - start_year + 1}
    ...
    def _get_popular_scenarios_detailed(self, user, year=None):
            ...
            scenarios.append(
                {
                    ...
                    "description": (
                        item["scenario__summary"][:200] + "..."
                        if len(item["scenario__summary"] or "") > 200
                        else item["scenario__summary"]
                    ),
                    "stats": {
                    ...
                    "dates": {
                        ...
                        "days_between": (
                            (item["last_played"] - item["first_played"]).days
                            if item["first_played"] and item["last_played"]
                            else 0
                        ),
                    },
                }
        ...
        return {"year": year if year else "all_time", "count": len(scenarios), "scenarios": scenarios}
    ...
    def _get_system_trends_detailed(self, user):
            ...
            for year in range(start_year, current_year + 1):
                if year not in existing_years:
                    system_data["yearly_data"].append(
                        {"year": year, "sessions": 0, "unique_scenarios": 0, "as_gm": 0, "as_player": 0}
                    )
        ...
        return {"period": f"{start_year} - {current_year}", "systems": list(yearly_system_data.values())}
