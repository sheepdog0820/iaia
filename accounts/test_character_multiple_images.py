...
class CharacterImageModelTestCase(TestCase):
    ...
    def create_test_image(self, name="test.png", size=(100, 100)):
        ...
        image.save(file, "PNG")
        file.seek(0)
        return SimpleUploadedFile(name=name, content=file.read(), content_type="image/png")
...
class CharacterImageAPITestCase(APITestCase):
    ...
    def test_upload_single_image(self):
        ...
        response = self.client.post(url, {"image": image, "is_main": True}, format="multipart")
    ...
    def test_upload_multiple_images(self):
        ...
        for i, image in enumerate(images):
            response = self.client.post(url, {"image": image, "order": i}, format="multipart")
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_invalid_file_format(self):
        """無効なファイル形式のテスト"""
        # テキストファイルをアップロード
        invalid_file = SimpleUploadedFile(name="test.txt", content=b"This is not an image", content_type="text/plain")
        url = reverse("character-image-list", kwargs={"character_id": self.character.id})
    ...
    def test_set_main_image(self):
        ...
        # img2をメイン画像に設定
        url = reverse("character-image-set-main", kwargs={"character_id": self.character.id, "pk": img2.id})
...
class CharacterImageIntegrationTestCase(TestCase):
    ...
    def test_drag_and_drop_upload(self):
        ...
        self.assertContains(response, "character-image-select-btn")
        self.assertContains(response, "character-image-modal-list")
