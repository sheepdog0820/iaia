...
class CharacterImageUploadTestCase(APITestCase):
    ...
    def create_test_image(self, filename="test_image.jpg", size=(400, 600), format="JPEG"):
        ...
        with open(temp_file.name, "rb") as f:
            uploaded_file = SimpleUploadedFile(filename, f.read(), content_type=f"image/{format.lower()}")
    ...
    def test_image_file_validation_invalid_format(self):
        """バリデーション: 不正な画像フォーマット"""
        # テキストファイルを画像として送信
        invalid_file = SimpleUploadedFile("not_image.txt", b"This is not an image", content_type="text/plain")
    ...
    def test_update_character_image(self):
        ...
        update_data = {"character_image": new_image}

        response = self.client.patch(f"/api/accounts/character-sheets/{character.id}/", update_data, format="multipart")
    ...
    def test_character_image_permissions(self):
        ...
        update_data = {"character_image": test_image}
    ...
    def test_character_list_includes_image_url(self):
        ...
        # 画像URLが含まれているか確認
        character_with_image = next((char for char in characters if char.get("character_image")), None)
