...
def sync_subscription_object(subscription, event_id=''):
    ...
    record = PremiumSubscription.objects.select_related('user').filter(
    ...
    cancel_at_period_end = bool(
        stripe_object_get(subscription, 'cancel_at_period_end', False)
    )
    ...
    if (
        status in PremiumSubscription.ACTIVE_STATUSES
        and not (record.revoked_at and previous_access_source == 'stripe')
    ):
...
def handle_checkout_completed(session, event_id=''):
    ...
    subscription_id = (
        stripe_object_get(subscription, 'id')
...
def mark_refund_or_dispute(data_object, *, event_type, event_id=''):
    ...
    dispute_closed_without_loss = (
        event_type == 'charge.dispute.closed'
        and dispute_status not in {'lost', 'warning_closed'}
    )
    ...
    invoice_id = (
        stripe_object_get(data_object, 'invoice', '')
        or stripe_object_get(charge_obj, 'invoice', '')
    )
    payment_intent_id = (
        stripe_object_get(data_object, 'payment_intent', '')
        or stripe_object_get(charge_obj, 'payment_intent', '')
    )
    create_premium_audit_log(
        record.user,
        action=action,
...
def redeem_premium_access_code(user, raw_code):
    ...
    with transaction.atomic():
        access_code = PremiumAccessCode.objects.select_for_update().filter(
