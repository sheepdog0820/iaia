...
class SessionCharacterIntegrationTestCase(TestCase):
    ...
    def test_session_creation_to_character_registration_flow(self):
        ...
        # グループ作成
        group = Group.objects.create(name="テストグループ", created_by=self.gm_user, visibility="private")
...
class MultiCharacterSessionTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        for i in range(4):
            user = User.objects.create_user(
                f"player{i+1}", "pass123", f"player{i+1}@test.com", nickname=f"プレイヤー{i+1}"
            )
            self.players.append(user)

        # シナリオとセッション作成
        self.scenario = Scenario.objects.create(
            title="深淵の呼び声", game_system="coc", recommended_players="3-5人", created_by=self.gm
        )
    ...
    def test_multiple_characters_simultaneous_participation(self):
        ...
        for participant_id in participants:
            # 参加者情報の取得確認
            response = self.client.get(f"/api/schedules/participants/{participant_id}/")
            self.assertEqual(response.status_code, status.HTTP_200_OK)

        # 参加者数の確認
        self.assertEqual(SessionParticipant.objects.filter(session=self.session).count(), 4)
...
class RealtimeStatusUpdateTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.scenario = Scenario.objects.create(title="テストシナリオ", game_system="coc", created_by=self.gm)
        ...
        self.session = TRPGSession.objects.create(
            title="進行中セッション", date=timezone.now(), gm=self.gm, group=self.group, status="ongoing"
        )
...
class SessionStatisticsUpdateTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # シナリオとセッション
        self.scenario = Scenario.objects.create(title="統計テストシナリオ", game_system="coc", created_by=self.gm)
    ...
    def test_session_completion_statistics(self):
        ...
        response = self.client.patch(
            f"/api/schedules/sessions/{self.session.id}/", {"status": "completed", "duration_minutes": 270}  # 4.5時間
        )
...
class CrossUserCollaborationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.users = []
        for i in range(5):
            user = User.objects.create_user(f"user{i+1}", "pass123", f"user{i+1}@test.com", nickname=f"ユーザー{i+1}")
        ...
        for i in range(3):
            gm = self.users[i]
            scenario = Scenario.objects.create(title=f"シナリオ{i+1}", game_system="coc", created_by=gm)

            group = Group.objects.create(name=f"グループ{i+1}", created_by=gm, visibility="public")
    ...
    def test_cross_participation_permissions(self):
        ...
        response = self.client.patch(f"/api/schedules/participants/{participant_id}/", {"status": "approved"})
    ...
    def test_multi_gm_collaboration(self):
        ...
        scenario = Scenario.objects.create(title="協力型シナリオ", game_system="coc", created_by=main_gm)

        group = Group.objects.create(name="協力GMグループ", created_by=main_gm, visibility="public")
...
class SessionCharacterSyncTestCase(TestCase):
    ...
    def test_character_deletion_handling(self):
        """キャラクター削除時のセッション参加処理"""
        # セッション作成と参加
        group = Group.objects.create(name="削除テストグループ", created_by=self.gm, visibility="private")
    ...
    def test_session_cancellation_notification(self):
        """セッションキャンセル時の通知"""
        # セッション作成
        group = Group.objects.create(name="キャンセルテストグループ", created_by=self.gm, visibility="private")
        ...
        response = self.client.patch(f"/api/schedules/sessions/{session.id}/", {"status": "cancelled"})
    ...
    def test_character_experience_tracking(self):
        ...
        for i in range(3):
            group = Group.objects.create(name=f"経験グループ{i+1}", created_by=self.gm, visibility="private")
