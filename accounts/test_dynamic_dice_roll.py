...
from accounts.models import CharacterDiceRollSetting, CustomUser
...
class DynamicDiceRollTestCase(TestCase):
    ...
    def test_validate_dice_settings(self):
        """ダイス設定のバリデーションテスト"""
        # 正常な設定
        valid_settings = {"count": 3, "sides": 6, "bonus": 0}
...
class DiceSettingUtilityFunctionTestCase(TestCase):
    ...
    def test_get_dice_count_function(self):
        ...
        setting = CharacterDiceRollSetting.objects.create(
            user=self.user, setting_name="テスト設定", str_dice_count=5, con_dice_count=3, pow_dice_count=2
        )
    ...
    def test_get_dice_sides_function(self):
        """ダイス面数取得関数のテスト"""
        setting = CharacterDiceRollSetting.objects.create(
            user=self.user, setting_name="テスト設定", str_dice_sides=8, con_dice_sides=10, pow_dice_sides=12
        )
    ...
    def test_get_dice_bonus_function(self):
        """ダイスボーナス取得関数のテスト"""
        setting = CharacterDiceRollSetting.objects.create(
            user=self.user, setting_name="テスト設定", str_bonus=5, con_bonus=-2, pow_bonus=0
        )
