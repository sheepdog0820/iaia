...
class SkillPointValidationTestCase(TestCase):
    ...
    def test_skill_point_over_allocation_validation(self):
        ...
        skill = CharacterSkill(
            character_sheet=self.character, skill_name="図書館", occupation_points=250  # 200を超える
        )
    ...
    def test_negative_skill_points_validation(self):
        """負の技能ポイントのバリデーション"""
        skill = CharacterSkill(character_sheet=self.character, skill_name="図書館", occupation_points=-10)
...
class SkillPointManagementAPITestCase(APITestCase):
    ...
    def test_allocate_skill_points(self):
        """技能ポイント割り振りAPIテスト"""
        # 既存技能にポイントを割り振る
        skill = CharacterSkill.objects.create(character_sheet=self.character, skill_name="図書館", base_value=25)
    ...
    def test_batch_skill_points_allocation(self):
        """一括技能ポイント割り振りAPIテスト"""
        # 複数の技能を作成
        skill1 = CharacterSkill.objects.create(character_sheet=self.character, skill_name="図書館", base_value=25)
        skill2 = CharacterSkill.objects.create(character_sheet=self.character, skill_name="目星", base_value=25)

        allocation_data = {
            "allocations": [
                {"skill_id": skill1.id, "occupation_points": 40, "interest_points": 10},
                {"skill_id": skill2.id, "occupation_points": 30, "interest_points": 15},
            ]
        }
...
class OccupationSkillSetTestCase(TestCase):
    ...
    def test_occupation_skill_set_template(self):
        ...
        expected_skills = ["医学", "応急手当", "生物学", "薬学", "心理学", "精神分析", "信用", "言いくるめ"]
