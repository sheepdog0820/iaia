...
class SimpleTindalosMetricsDetailedTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.group = Group.objects.create(
            name="Test Group", description="Test group for statistics", visibility="private", created_by=self.user
        )
        ...
        for year_offset in range(3):
            ...
            for month in [3, 6, 9, 12]:
                ...
                PlayHistory.objects.create(
                    scenario=scenario, user=self.user, session=session, played_date=date, role="gm"
                )
    ...
    def test_popular_scenarios_ranking(self):
        ...
        for scenario in self.scenarios[1:]:
            ...
            PlayHistory.objects.create(
                scenario=scenario, user=self.user, session=session, played_date=timezone.now(), role="player"
            )
    ...
    def test_system_trends_over_years(self):
        ...
        for year_offset in range(3):
            ...
            for i in range(3 + year_offset):  # 年を追うごとに増加
                ...
                PlayHistory.objects.create(
                    scenario=self.scenarios[0], user=self.user, session=session, played_date=date, role="gm"  # CoC
                )
            ...
            for i in range(5 - year_offset):  # 年を追うごとに減少
                ...
                PlayHistory.objects.create(
                    scenario=self.scenarios[1], user=self.user, session=session, played_date=date, role="player"  # D&D
                )
...
class DetailedTindalosMetricsTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        # テスト用データの作成
        self.group = Group.objects.create(
            name="Test Group", description="Test group", visibility="private", created_by=self.user
        )
        self.group.members.add(self.user)
    ...
    def test_summary_stats_endpoint(self):
        ...
        PlayHistory.objects.create(
            scenario=self.scenario, user=self.user, session=session, played_date=timezone.now(), role="gm"
        )
    ...
    def test_yearly_trends_endpoint(self):
        ...
        for year_offset in range(3):
            ...
            PlayHistory.objects.create(
                scenario=self.scenario, user=self.user, session=session, played_date=date, role="player"
            )
    ...
    def test_monthly_details_endpoint(self):
        ...
        for month in range(1, 4):  # 1-3月のみ
            ...
            PlayHistory.objects.create(
                scenario=self.scenario, user=self.user, session=session, played_date=date, role="gm"
            )
    ...
    def test_system_trends_endpoint(self):
        ...
        for year_offset in range(2):
            ...
            for system in systems:
                ...
                PlayHistory.objects.create(
                    scenario=scenario, user=self.user, session=session, played_date=datetime(year, 6, 15), role="gm"
                )
