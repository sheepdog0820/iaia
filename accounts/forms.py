...
class CustomLoginForm(AuthenticationForm):
    ...
    username = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "ユーザー名またはメールアドレス", "autofocus": True}
        ),
        label="ユーザー名またはメールアドレス",
    )

    password = forms.CharField(
        widget=forms.PasswordInput(attrs={"class": "form-control", "placeholder": "パスワード"}), label="パスワード"
    )

    remember_me = forms.BooleanField(
        required=False, widget=forms.CheckboxInput(attrs={"class": "form-check-input"}), label="ログイン状態を保持する"
    )
...
class CustomSignUpForm(UserCreationForm):
    ...
    nickname = forms.CharField(
        max_length=50,
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "ニックネーム（任意）"}),
        label="ニックネーム",
    )
    ...
    trpg_history = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={"class": "form-control", "rows": 3, "placeholder": "TRPG歴やゲーム経験について（任意）"}
        ),
        label="TRPG歴",
    )
...
class ProfileEditForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["trpg_scenario_structure"].initial = self._get_sheet_value(
            sheet, "scenario", "structure", default=""
        )
        self.fields["trpg_scenario_play_feel"].initial = self._get_sheet_value(
            sheet, "scenario", "play_feel", default=""
        )
        self.fields["trpg_scenario_volume"].initial = self._get_sheet_value(sheet, "scenario", "volume", default="")
        self.fields["trpg_story_preferences"].initial = self._get_sheet_value(
            sheet, "scenario", "preferences", default=[]
        )
        ...
        self.fields["trpg_loss_preference_note"].initial = self._get_sheet_value(
            sheet, "scenario", "loss_note", default=""
        )

        self.fields["trpg_rp_style"].initial = self._get_sheet_value(sheet, "character_play", "rp_style", default=[])
        self.fields["trpg_role_preference"].initial = self._get_sheet_value(
            sheet, "character_play", "role_preference", default=[]
        )
        ...
        self.fields["trpg_ng_expression_other"].initial = self._get_sheet_value(
            sheet, "ng", "expression_other", default=""
        )
        ...
        self.fields["trpg_direction_amount"].initial = self._get_sheet_value(
            sheet, "session", "direction_amount", default=""
        )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "first_name": forms.TextInput(attrs={"class": "form-control", "placeholder": "名前"}),
            "last_name": forms.TextInput(attrs={"class": "form-control", "placeholder": "姓"}),
            "trpg_history": forms.Textarea(
                attrs={"class": "form-control", "rows": 4, "placeholder": "TRPG歴やゲーム経験について"}
            ),
...
class CharacterSheet6thForm(forms.ModelForm):
    ...
    mental_disorder = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"class": "form-control", "rows": 3, "placeholder": "精神的障害の詳細（任意）"}),
        label="精神的障害",
    )

    # 複数画像アップロード
    character_images = MultipleFileField(required=False, label="キャラクター画像（複数選択可能）")
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(attrs={"class": "form-control", "placeholder": "探索者名", "required": True}),
            "player_name": forms.TextInput(attrs={"class": "form-control", "placeholder": "プレイヤー名"}),
            "age": forms.NumberInput(attrs={"class": "form-control", "min": 15, "max": 90, "placeholder": "年齢"}),
            "gender": forms.TextInput(attrs={"class": "form-control", "placeholder": "性別"}),
            "occupation": forms.TextInput(attrs={"class": "form-control", "placeholder": "職業"}),
            "birthplace": forms.TextInput(attrs={"class": "form-control", "placeholder": "出身地"}),
            "residence": forms.TextInput(attrs={"class": "form-control", "placeholder": "現住所"}),
            "str_value": forms.NumberInput(
                attrs={
            ...
            "notes": forms.Textarea(attrs={"class": "form-control", "rows": 4, "placeholder": "探索者の背景や特徴"}),
            "hit_points_max": forms.NumberInput(attrs={"class": "form-control bg-light", "readonly": True}),
            "hit_points_current": forms.NumberInput(attrs={"class": "form-control", "placeholder": "現在HP"}),
            "magic_points_max": forms.NumberInput(attrs={"class": "form-control bg-light", "readonly": True}),
            "magic_points_current": forms.NumberInput(attrs={"class": "form-control", "placeholder": "現在MP"}),
            "sanity_starting": forms.NumberInput(attrs={"class": "form-control bg-light", "readonly": True}),
            "sanity_max": forms.NumberInput(attrs={"class": "form-control bg-light", "readonly": True}),
            "sanity_current": forms.NumberInput(attrs={"class": "form-control", "placeholder": "現在正気度"}),
        }
        labels = {
            "name": "探索者名",
            "player_name": "プレイヤー名",
    ...
    def clean(self):
        ...
        if len(image_files) > image_limit:
            raise forms.ValidationError({"character_images": character_image_limit_error_message(image_limit)})
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            sixth_data, created = CharacterSheet6th.objects.get_or_create(
                character_sheet=instance, defaults={"mental_disorder": self.cleaned_data.get("mental_disorder", "")}
            )
            if not created:
