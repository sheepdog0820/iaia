...
class StatisticsViewsTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        # グループ作成
        self.group = Group.objects.create(name="Test Group", description="Test Description", created_by=self.user1)
        ...
        for i in range(5):
            ...
            PlayHistory.objects.create(
                scenario=self.scenario, user=self.user2, session=session, played_date=session.date, role="player"
            )
    ...
    def test_user_ranking_trends_monthly(self):
        ...
        response = self.client.get(
            f"/api/accounts/statistics/ranking/?year={year}&type=sessions&trends=true&trend_unit=month&limit=5"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
