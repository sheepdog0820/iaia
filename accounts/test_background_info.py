...
from .character_models import CharacterBackground, CharacterSheet6th
...
class BackgroundModelTestCase(TestCase):
    ...
    def test_background_field_validation(self):
        ...
        with self.assertRaises(ValidationError):
            background = CharacterBackground(character_sheet=self.character, appearance_description=long_text)
...
class BackgroundAPITestCase(APITestCase):
    ...
    def test_get_background_summary_api(self):
        ...
        response = self.client.get(f"/accounts/character-sheets/{self.character.id}/background-summary/")
    ...
    def test_update_background_data_api(self):
        ...
        # 既存の背景情報を作成
        background = CharacterBackground.objects.create(
            character_sheet=self.character, appearance_description="初期の容姿"
        )
        ...
        response = self.client.patch(
            f"/accounts/character-sheets/{self.character.id}/update-background-data/", update_data, format="json"
        )
    ...
    def test_create_background_if_not_exists(self):
        ...
        update_data = {"appearance_description": "新規作成された容姿", "beliefs_ideology": "新規作成された信念"}
