...
class CharacterIntegrationTestCase(TestCase):
    ...
    def test_character_creation_flow(self):
        ...
        character_data = {
            ...
            "skills_data": json.dumps(
                [
                    {"skill_name": "コンピューター", "base_value": 1, "occupation_points": 50, "interest_points": 20},
                    {"skill_name": "図書館", "base_value": 25, "occupation_points": 30, "interest_points": 10},
                ]
            ),
        }

        response = self.api_client.post("/api/accounts/character-sheets/", character_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        character_id = response.data["id"]
        print(f"OK キャラクター作成成功: ID={character_id}")

        # 作成後に詳細を取得して副次ステータスを確認
        detail_response = self.api_client.get(f"/api/accounts/character-sheets/{character_id}/")
    ...
    def test_character_view_and_edit(self):
        ...
        # 1. 詳細画面にアクセス
        response = self.client.get(reverse("character_detail_6th", kwargs={"character_id": character_id}))
        self.assertEqual(response.status_code, 200)
        print("OK 詳細画面: 正常表示")

        # 2. API経由で詳細データ取得
        response = self.api_client.get(f"/api/accounts/character-sheets/{character_id}/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["name"], "統合テスト探索者")
        print("OK API詳細取得: 正常")

        # 3. 編集画面にアクセス（作成画面にIDパラメータ付き）
        response = self.client.get(reverse("character_create_6th") + f"?id={character_id}")
        self.assertEqual(response.status_code, 200)
        print("OK 編集画面: 正常表示")
        ...
        response = self.api_client.patch(f"/api/accounts/character-sheets/{character_id}/", update_data, format="json")
        self.assertEqual(response.status_code, 200)
    ...
    def test_character_image_management(self):
        ...
        response = self.api_client.post(
            f"/api/accounts/character-sheets/{character.id}/images/", {"image": additional_image}, format="multipart"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        print("OK 追加画像アップロード: 成功")

        # 3. 画像一覧を取得
        response = self.api_client.get(f"/api/accounts/character-sheets/{character.id}/images/")
        self.assertEqual(response.status_code, 200)
        ...
        # 5. 画像削除
        response = self.api_client.delete(f"/api/accounts/character-sheets/{character.id}/images/{image_id}/")
    ...
    def test_character_version_management(self):
        ...
        # 3. さらに新しいバージョンを作成
        response = self.api_client.post(f"/api/accounts/character-sheets/{version2_id}/create_version/")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        # 4. バージョン履歴の取得
        response = self.api_client.get(f"/api/accounts/character-sheets/{original.id}/versions/")
        self.assertEqual(response.status_code, 200)
    ...
    def test_ccfolia_export(self):
        ...
        # CCFOLIA形式でエクスポート
        response = self.api_client.get(f"/api/accounts/character-sheets/{character.id}/ccfolia_json/")
        self.assertEqual(response.status_code, 200)
...
class CharacterAPIPermissionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.public_character = self._create_character_with_stats(
            user=self.user1, name="ユーザー1の公開キャラクター", player_name="ユーザー1", age=30, access_scope="public"
        )
    ...
    def test_own_character_access(self):
        ...
        # 非公開キャラクターの詳細取得
        response = self.api_client.get(f"/api/accounts/character-sheets/{self.private_character.id}/")
        self.assertEqual(response.status_code, 200)
        print("OK other user non-public character: not visible")

        # 公開キャラクターの詳細取得
        response = self.api_client.get(f"/api/accounts/character-sheets/{self.public_character.id}/")
        self.assertEqual(response.status_code, 200)
    ...
    def test_other_user_character_access(self):
        ...
        # 削除も拒否
        response = self.api_client.delete(f"/api/accounts/character-sheets/{self.public_character.id}/")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class CharacterAdvancedIntegrationTestCase(TestCase):
    ...
    def test_character_status_management(self):
        ...
        for status, name in status_changes:
            response = self.api_client.patch(
                f"/api/accounts/character-sheets/{character.id}/", {"status": status}, format="json"
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_ability_score_boundaries(self):
        ...
        response = self.api_client.post("/api/accounts/character-sheets/", min_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.api_client.post("/api/accounts/character-sheets/", max_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.api_client.post("/api/accounts/character-sheets/", invalid_data, format="json")
        # バリデーションが緩い場合は成功する
    ...
    def test_character_search_and_filter(self):
        ...
        # 1. 名前で検索
        response = self.api_client.get("/api/accounts/character-sheets/", {"search": "検索テスト"})
        ...
        # 2. ステータスでフィルター
        response = self.api_client.get("/api/accounts/character-sheets/", {"status": "alive"})
        if "results" in response.data:
            results = response.data["results"]
        else:
            results = response.data
    ...
    def test_session_character_integration(self):
        ...
        group = Group.objects.create(
            name="テストグループ", description="テスト用グループ", visibility="private", created_by=self.user
        )
