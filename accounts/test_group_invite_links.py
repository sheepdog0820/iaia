...
class GroupInviteLinkAPITestCase(APITestCase):
    ...
    def test_invite_link_respects_max_uses(self):
        ...
        self.assertFalse(GroupMembership.objects.filter(user=other, group=self.group).exists())
    ...
    def test_revoked_invite_link_cannot_be_used(self):
        ...
        token = issued.data["token"]
        self.client.force_authenticate(user=self.admin)
        revoke = self.client.delete(f"/api/accounts/groups/{self.group.id}/invite-links/{link_id}/")
