...
class SkillGrowthRecordModelTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.growth_record = GrowthRecord.objects.create(
            character_sheet=self.character, session_date=date(2024, 1, 15), scenario_name="テストシナリオ"
        )
    ...
    def test_skill_growth_validation(self):
        ...
        with self.assertRaises(ValidationError):
            skill_growth = SkillGrowthRecord(
                growth_record=self.growth_record, skill_name="目星", old_value=95, new_value=98  # 90を超える値
            )
...
class GrowthRecordAPITestCase(APITestCase):
    ...
    def test_create_growth_record_api(self):
        ...
        response = self.client.post(
            f"/accounts/character-sheets/{self.character.id}/growth-records/", growth_data, format="json"
        )
    ...
    def test_get_growth_records_list_api(self):
        ...
        GrowthRecord.objects.create(
            character_sheet=self.character, session_date=date(2024, 1, 15), scenario_name="影の館", sanity_lost=5
        )
        GrowthRecord.objects.create(
            character_sheet=self.character, session_date=date(2024, 2, 20), scenario_name="古い屋敷", sanity_lost=3
        )

        response = self.client.get(f"/accounts/character-sheets/{self.character.id}/growth-records/")
    ...
    def test_add_skill_growth_to_record_api(self):
        ...
        # データベースに保存されたか確認
        skill_growth = SkillGrowthRecord.objects.get(growth_record=growth_record, skill_name="目星")
        self.assertEqual(skill_growth.growth_amount, 5)
    ...
    def test_get_growth_summary_api(self):
        ...
        response = self.client.get(f"/accounts/character-sheets/{self.character.id}/growth-summary/")
...
class GrowthRecordIntegrationTestCase(TestCase):
    ...
    def test_multiple_sessions_progression(self):
        ...
        # 複数セッションを作成
        sessions_data = [
            {"date": date(2024, 1, 15), "scenario": "影の館", "sanity_lost": 5, "experience": 3},
            {"date": date(2024, 2, 20), "scenario": "古い屋敷", "sanity_lost": 3, "experience": 2},
            {"date": date(2024, 3, 10), "scenario": "深海の秘密", "sanity_lost": 8, "experience": 4},
        ]
