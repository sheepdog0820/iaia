...
class ExportFunctionalityTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.group = Group.objects.create(
            name="Export Test Group", description="Test Group for Export", created_by=self.user
        )
        GroupMembership.objects.create(user=self.user, group=self.group, role="admin")

        # シナリオ作成
        self.scenario = Scenario.objects.create(
            title="Export Test Scenario", author="Export Author", game_system="coc", created_by=self.user
        )
    ...
    def test_export_available_formats_view(self):
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            if isinstance(formats, dict):
                ...
                format_names = [fmt.get("format", fmt.get("name")) for fmt in formats if isinstance(fmt, dict)]
...
class ExportIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.scenario = Scenario.objects.create(
            title="Integration Test Scenario", author="Test Author", game_system="coc", created_by=self.gm_user
        )
