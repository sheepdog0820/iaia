...
class ItemManagementTestCase(TestCase):
    ...
    def test_item_quantity_management(self):
        ...
        # 複数個のアイテムを作成
        item = CharacterEquipment.objects.create(
            character_sheet=self.character, item_type="item", name="懐中電灯", quantity=3, weight=0.5
        )
    ...
    def test_item_validation(self):
        ...
        with self.assertRaises(ValidationError):
            item = CharacterEquipment(
                character_sheet=self.character, item_type="item", name="テストアイテム", quantity=-1
            )
            item.full_clean()
        ...
        with self.assertRaises(ValidationError):
            item = CharacterEquipment(
                character_sheet=self.character, item_type="item", name="テストアイテム", weight=-1.0
            )
            item.full_clean()
    ...
    def test_multiple_items_management(self):
        ...
        for item_data in items_data:
            CharacterEquipment.objects.create(character_sheet=self.character, item_type="item", **item_data)

        # 全アイテムの確認
        items = CharacterEquipment.objects.filter(character_sheet=self.character, item_type="item")
        self.assertEqual(items.count(), 4)

        # 総重量の計算
        total_weight = sum(item.weight * item.quantity for item in items if item.weight is not None)
...
class InventoryAPITestCase(APITestCase):
    ...
    def test_get_inventory_summary_api(self):
        ...
        CharacterEquipment.objects.create(
            character_sheet=self.character, item_type="item", name="懐中電灯", quantity=2, weight=0.5
        )
        CharacterEquipment.objects.create(
    ...
    def test_bulk_update_items_api(self):
        ...
        item1 = CharacterEquipment.objects.create(
            character_sheet=self.character, item_type="item", name="懐中電灯", quantity=2
        )
        item2 = CharacterEquipment.objects.create(
            character_sheet=self.character, item_type="item", name="ロープ", quantity=1
        )
...
class InventoryIntegrationTestCase(TestCase):
    ...
    def test_complete_inventory_setup(self):
        ...
        equipment_data = [
            ...
            # 防具
            {"item_type": "armor", "name": "トレンチコート", "quantity": 1, "weight": 2.0, "armor_points": 1},
            # 一般アイテム
            {"item_type": "item", "name": "虫眼鏡", "quantity": 1, "weight": 0.1, "description": "目星判定に+5%"},
            {"item_type": "item", "name": "ノートと鉛筆", "quantity": 1, "weight": 0.2, "description": "調査記録用"},
            {"item_type": "item", "name": "懐中電灯", "quantity": 2, "weight": 0.5, "description": "予備電池付き"},
        ]

        created_items = []
        for item_data in equipment_data:
            item = CharacterEquipment.objects.create(character_sheet=self.character, **item_data)
            created_items.append(item)

        # 全装備の確認
        all_equipment = CharacterEquipment.objects.filter(character_sheet=self.character)
        ...
        # 総重量の計算
        total_weight = sum(item.weight * item.quantity for item in all_equipment if item.weight is not None)
