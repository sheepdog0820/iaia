...
def assign_demo_data_to_mock_users():
    ...
    mock_users = User.objects.filter(email__in=["demo.google@example.com", "demo.twitter@example.com"])
    ...
    for user in mock_users:
        ...
        for session in sessions[:3]:  # 最初の3つのセッションに参加
            participant, created = SessionParticipant.objects.get_or_create(
                session=session,
                user=user,
                defaults={"role": "player", "character_name": f"探索者{user.nickname or user.username}"},
            )
            if created:
        ...
        for i, scenario in enumerate(scenarios):
            history, created = PlayHistory.objects.get_or_create(
                ...
                defaults={"role": "player", "notes": f"{scenario.title}を楽しくプレイしました！"},
            )
            if created:
