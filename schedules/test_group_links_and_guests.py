...
from schedules.models import GuestClaimAudit, GuestInvitation, HandoutInfo, SessionParticipant, TRPGSession
...
class GroupLinkTestCase(APITestCase):
    def setUp(self):
        ...
        GroupMembership.objects.create(group=self.target, user=self.target_member, role="member")
    ...
    def test_mutual_approval_and_explicit_share_boundary(self):
        ...
        accepted = self.client.post(f"/api/groups/{self.target.pk}/links/{link_id}/accept/")
        ...
        results = sessions.data.get("results", sessions.data) if isinstance(sessions.data, dict) else sessions.data
    ...
    def test_only_target_admin_can_accept(self):
        ...
        self.client.force_authenticate(self.target_member)
        response = self.client.post(f"/api/groups/{self.target.pk}/links/{link.pk}/accept/")
...
class GuestInvitationClaimTestCase(APITestCase):
    ...
    def test_claim_preserves_slot_character_and_handout_assignment(self):
        ...
        self.client.force_authenticate(self.gm)
        delete_response = self.client.delete(f"/api/schedules/sessions/{self.session.pk}/")
    ...
    def test_revoked_invitation_cannot_be_used(self):
        ...
        token = issued.data["token"]
        revoked = self.client.delete(f'/api/sessions/{self.session.pk}/guest-invitations/{issued.data["id"]}/')
    ...
    def test_expired_invitation_landing_does_not_expose_session_details(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_410_GONE)
        self.assertContains(
            response, "この招待は期限切れ、失効済み、または使用済みです。", status_code=status.HTTP_410_GONE
        )
    ...
    def test_group_admin_can_issue_and_revoke_guest_invitation(self):
        ...
        revoked = self.client.delete(f"/api/sessions/{self.session.pk}/guest-invitations/{invitation.pk}/")
        self.assertEqual(revoked.status_code, status.HTTP_204_NO_CONTENT)
