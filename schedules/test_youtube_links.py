...
class SessionYouTubeLinkModelTestCase(TestCase):
    ...
    def test_extract_video_id_invalid_url(self):
        ...
        invalid_urls = ["https://www.google.com", "not_a_url", "https://vimeo.com/123456", "", None]
...
class SessionYouTubeLinkAPITestCase(APITestCase):
    ...
    def test_gm_can_add_youtube_link(self, mock_fetch):
        ...
        response = self.client.post(f"/api/schedules/sessions/{self.session.id}/youtube-links/", data, format="json")
    ...
    def test_non_participant_cannot_add_youtube_link(self):
        ...
        data = {"youtube_url": "https://youtube.com/watch?v=forbidden", "description": "追加できないはず"}
    ...
    def test_fetch_video_info_endpoint(self):
        ...
        with patch("schedules.services.YouTubeService.fetch_video_info") as mock_fetch:
            ...
            data = {"youtube_url": "https://www.youtube.com/watch?v=fetched123"}

            response = self.client.post("/api/schedules/youtube-links/fetch_info/", data, format="json")
    ...
    def test_update_youtube_link(self):
        ...
        data = {"description": "更新された備考"}

        response = self.client.patch(f"/api/schedules/youtube-links/{link.id}/", data, format="json")
        ...
        response = self.client.patch(
            f"/api/schedules/youtube-links/{link.id}/", {"description": "不正な更新"}, format="json"
        )
    ...
    def test_reorder_youtube_links(self):
        ...
        # GMが順序変更
        self.client.force_authenticate(user=self.gm)
        response = self.client.post(f"/api/schedules/youtube-links/{link3.id}/reorder/", {"order": 1}, format="json")
        ...
        # プレイヤーは順序変更できない
        self.client.force_authenticate(user=self.player1)
        response = self.client.post(f"/api/schedules/youtube-links/{link2.id}/reorder/", {"order": 3}, format="json")
    ...
    def test_list_youtube_links_for_session(self):
        ...
        self.client.force_authenticate(user=self.gm)
        response = self.client.get(f"/api/schedules/sessions/{self.session.id}/youtube-links/")
...
class YouTubeServiceTestCase(TestCase):
            ...
            "items": [
                {
                    "snippet": {
                        ...
                        "thumbnails": {"maxres": {"url": "https://i.ytimg.com/vi/test/maxresdefault.jpg"}},
                    },
                    "contentDetails": {"duration": "PT3M33S"},  # 3分33秒
                }
            ]
        }
    ...
    def test_fetch_video_info_not_found(self, mock_get, mock_getattr):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"items": []}  # 動画が見つからない
        mock_get.return_value = mock_response
...
class YouTubeLinkStatisticsAPITestCase(APITestCase):
    ...
    def test_get_statistics_as_gm(self):
        ...
        response = self.client.get(f"/api/schedules/sessions/{self.session.id}/youtube-links/statistics/")
