...
class ScheduleAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.scenario = Scenario.objects.create(title="Linked Scenario", game_system="coc", created_by=self.user1)
    ...
    def test_session_viewset_list_period_filter(self):
        ...
        default_sessions = (
            default_payload.get("results", default_payload) if isinstance(default_payload, dict) else default_payload
        )
        ...
        past_sessions = past_payload.get("results", past_payload) if isinstance(past_payload, dict) else past_payload
    ...
    def test_session_creation_via_api(self):
        ...
        self.assertTrue(
            SessionParticipantRole.objects.filter(
                participant__session=created_session,
                participant__user=self.user1,
                role=SessionParticipantRole.Role.OWNER,
            ).exists()
        )
        self.assertFalse(session_permissions.can_view_secret_content(self.user1, created_session))
    ...
    def test_session_creation_copies_scenario_handouts(self):
        ...
        self.client.force_authenticate(user=self.user2)
        context_response = self.client.get(f"/api/schedules/sessions/next-context/?session_id={created_session.id}")
    ...
    def test_manager_linking_internal_character_uses_sheet_name(self):
        participant = session_permissions.create_participant(
            session=self.session,
            user=self.user2,
            role="player",
        ...
        response = self.client.patch(
            f"/api/schedules/participants/{participant.id}/",
            {
                "character_name": "Ignored Manual Name",
    ...
    def test_joining_with_internal_character_uses_sheet_name(self):
        ...
        response = self.client.post(
            ...
            {
                "character_name": "Ignored Join Name",
                "character_sheet_id": character.id,
            },
            format="json",
        )
    ...
    def test_main_gm_participant_cannot_be_deleted(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["error"], "GMは除名できません。")
    ...
    def test_guest_participant_invalid_tableno_character_url_keeps_manual_name(self):
        ...
        response = self.client.patch(
            f"/api/schedules/participants/{participant.id}/",
            {
                "character_name": "Manual Guest PC",
                "character_sheet_url": "https://tableno.example/api/accounts/character-sheets/not-a-number/",
            },
            format="json",
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        participant.refresh_from_db()
        self.assertEqual(participant.character_name, "Manual Guest PC")
        self.assertEqual(response.data["character_name"], "Manual Guest PC")
    ...
    def test_calendar_api(self):
        ...
        response = self.client.get(f"/api/schedules/calendar/?start={start_date}&end={end_date}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_session_invitation_accept_creates_participant(self):
        ...
        invite_res = self.client.post(
            f"/api/schedules/sessions/{self.session.id}/invite/", {"user_id": self.user2.id}, format="json"
        )
...
class PublicSessionLinkTestCase(APITestCase):
    ...
    def test_fixed_session_share_view_accessible_without_login(self):
        self.session.visibility = "public"
        self.session.save(update_fields=["visibility"])

        url = reverse("fixed-shared-session-view", kwargs={"share_token": self.session.share_token})
    ...
    def test_fixed_session_share_view_invalid_token_404(self):
        url = reverse("fixed-shared-session-view", kwargs={"share_token": uuid.uuid4()})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
