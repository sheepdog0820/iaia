...
class SessionManagementIntegrationTestCase(APITestCase):
    ...
    def test_session_managers_can_add_guest_character_and_rewards_for_archived_session(self):
        ...
        response = self.client.post(
            reverse("session-rewards", kwargs={"session_id": session_id}),
            {
    ...
    def test_complete_session_lifecycle(self):
        ...
        for player in self.players[:2]:
            response = self.client.post(invite_url, {"user_id": player.id}, format="json")
            self.assertEqual(response.status_code, status.HTTP_200_OK)

            # 通知確認
            notification = HandoutNotification.objects.filter(
                recipient=player, notification_type="session_invitation"
            ).first()
        ...
        for player in self.players[:2]:
            ...
            response = self.client.post(join_url, {"character_name": f"{player.nickname}のキャラ"}, format="json")
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.client.patch(update_url, {"date": new_date.isoformat()}, format="json")
        ...
        for player in self.players[:2]:
            notification = HandoutNotification.objects.filter(
                recipient=player, notification_type="schedule_change"
            ).latest("created_at")
            self.assertIsNotNone(notification)
        ...
        self.client.force_authenticate(user=self.gm)
        response = self.client.patch(
            update_url, {"status": "cancelled", "cancel_reason": "都合により中止"}, format="json"
        )
        ...
        for player in self.players[:2]:
            notification = HandoutNotification.objects.filter(
                recipient=player, notification_type="session_cancelled"
            ).first()
            self.assertIsNotNone(notification)
...
class NotificationSystemIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        # グループ作成
        self.group = Group.objects.create(name="通知テストグループ", created_by=self.users[0])
    ...
    def test_notification_preferences_integration(self):
        ...
        for user in self.users[1:]:
            response = self.client.post(invite_url, {"user_id": user.id}, format="json")
        ...
        notif1 = HandoutNotification.objects.filter(
            recipient=self.users[1], notification_type="session_invitation"
        ).count()
        self.assertEqual(notif1, 1)

        # user2: 通知なし（セッション通知無効）
        notif2 = HandoutNotification.objects.filter(
            recipient=self.users[2], notification_type="session_invitation"
        ).count()
        self.assertEqual(notif2, 0)

        # user3: 通知なし（全通知無効）
        notif3 = HandoutNotification.objects.filter(
            recipient=self.users[3], notification_type="session_invitation"
        ).count()
        ...
        # 既読にマーク
        notification = HandoutNotification.objects.filter(recipient=self.users[1]).first()
...
class CalendarExportIntegrationTestCase(APITestCase):
    ...
    def test_calendar_permission_isolation(self):
        ...
        monthly_url = reverse("monthly_events")
        response = self.client.get(monthly_url, {"month": timezone.now().strftime("%Y-%m")})
