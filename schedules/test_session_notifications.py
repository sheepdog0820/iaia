...
class SessionNotificationServiceTestCase(TestCase):
    ...
    def test_send_session_invitation_notification(self):
        """セッション招待通知の送信テスト"""
        # 通知送信
        result = self.notification_service.send_session_invitation_notification(self.session, self.gm, self.player2)

        self.assertTrue(result)

        # 通知確認
        notification = HandoutNotification.objects.filter(
            recipient=self.player2, notification_type="session_invitation"
        ).first()
    ...
    def test_send_session_schedule_change_notification(self):
        ...
        # 通知送信
        count = self.notification_service.send_session_schedule_change_notification(self.session, old_date, new_date)

        self.assertEqual(count, 1)  # player1のみ

        # 通知確認
        notification = HandoutNotification.objects.filter(
            recipient=self.player1, notification_type="schedule_change"
        ).first()
        ...
        gm_notifications = HandoutNotification.objects.filter(
            recipient=self.gm, notification_type="schedule_change"
        ).count()
    ...
    def test_send_session_cancelled_notification(self):
        """セッションキャンセル通知の送信テスト"""
        # 通知送信（理由あり）
        count = self.notification_service.send_session_cancelled_notification(self.session, reason="体調不良のため")

        self.assertEqual(count, 1)

        # 通知確認
        notification = HandoutNotification.objects.filter(
            recipient=self.player1, notification_type="session_cancelled"
        ).first()
    ...
    def test_send_session_reminder_notification(self):
        ...
        # 通知確認
        notifications = HandoutNotification.objects.filter(notification_type="session_reminder")
...
class SessionInviteAPITestCase(APITestCase):
    ...
    def test_invite_success(self):
        ...
        # 通知確認
        notification = HandoutNotification.objects.filter(
            recipient=self.non_member, notification_type="session_invitation"
        ).first()
        self.assertIsNotNone(notification)
...
class SessionUpdateNotificationTestCase(APITestCase):
    ...
    def test_schedule_change_notification_on_update(self):
        ...
        data = {"title": self.session.title, "date": new_date, "group": self.group.id, "status": "planned"}
        ...
        # 通知確認
        notification = HandoutNotification.objects.filter(
            recipient=self.player, notification_type="schedule_change"
        ).first()
    ...
    def test_cancellation_notification_on_status_change(self):
        ...
        data = {"status": "cancelled", "cancel_reason": "会場が確保できなかったため"}
        ...
        # 通知確認
        notification = HandoutNotification.objects.filter(
            recipient=self.player, notification_type="session_cancelled"
        ).first()
    ...
    def test_no_notification_on_minor_update(self):
        ...
        notifications = HandoutNotification.objects.filter(
            recipient=self.player, notification_type__in=["schedule_change", "session_cancelled"]
        )
