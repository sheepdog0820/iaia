...
class HandoutAttachmentModelTest(TestCase):
    ...
    def setUp(self):
        """テストデータの準備"""
        # ユーザー作成
        self.gm_user = CustomUser.objects.create_user(username="gm_user", email="gm@example.com", nickname="テストGM")
        self.player1 = CustomUser.objects.create_user(
            username="player1", email="player1@example.com", nickname="プレイヤー1"
        )

        # グループ作成
        self.group = Group.objects.create(
            name="テストグループ", description="テスト用のグループ", created_by=self.gm_user, visibility="private"
        )
        ...
        self.participant1 = SessionParticipant.objects.create(
            session=self.session, user=self.player1, role="player", character_name="探索者1"
        )
    ...
    def test_handout_attachment_model_creation(self):
        ...
        test_file = SimpleUploadedFile("test_image.jpg", image_io.getvalue(), content_type="image/jpeg")
    ...
    def test_file_type_validation(self):
        ...
        with self.assertRaises(ValidationError):
            attachment = HandoutAttachment(handout=self.handout, file_type="invalid_type", uploaded_by=self.gm_user)
    ...
    def test_file_upload_path(self):
        ...
        attachment = HandoutAttachment(
            handout=self.handout, original_filename="test_file.jpg", file_type="image", uploaded_by=self.gm_user
        )
...
class HandoutAttachmentServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.group = Group.objects.create(name="テストグループ", created_by=self.gm_user, visibility="private")
        ...
        self.participant1 = SessionParticipant.objects.create(session=self.session, user=self.player1, role="player")
    ...
    def test_upload_image_attachment(self):
        ...
        attachment = service.upload_attachment(handout=self.handout, file=test_file, uploaded_by=self.gm_user)
    ...
    def test_file_size_limit_validation(self):
        ...
        # 大きすぎるファイルのシミュレーション
        large_file = SimpleUploadedFile("large_file.jpg", b"x" * (20 * 1024 * 1024), content_type="image/jpeg")  # 20MB

        with self.assertRaises(ValidationError):
            service.upload_attachment(handout=self.handout, file=large_file, uploaded_by=self.gm_user)
    ...
    def test_unsupported_file_type_validation(self):
        ...
        unsupported_file = SimpleUploadedFile(
            "test.exe", b"fake executable content", content_type="application/x-msdownload"
        )

        with self.assertRaises(ValidationError):
            service.upload_attachment(handout=self.handout, file=unsupported_file, uploaded_by=self.gm_user)
    ...
    def test_attachment_permission_check(self):
        ...
        # 権限のないユーザー
        other_user = CustomUser.objects.create_user(username="other_user", email="other@example.com")
...
class HandoutAttachmentAPITest(TestCase):
    ...
    def setUp(self):
        ...
        self.handout = HandoutInfo.objects.create(
            session=self.session,
            participant=SessionParticipant.objects.create(session=self.session, user=self.player1, role="player"),
            title="テストハンドアウト",
            content="テスト内容",
            is_secret=True,
        )
    ...
    def create_test_image(self):
        ...
        return SimpleUploadedFile("api_test_image.jpg", image_io.getvalue(), content_type="image/jpeg")
    ...
    def test_attachment_upload_api(self):
        ...
        response = client.post(
            ...
            {"file": test_file, "description": "テスト用画像ファイル"},
        )
