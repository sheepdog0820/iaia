...
class HandoutNotificationModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.player2 = CustomUser.objects.create_user(
            username="player2", email="player2@example.com", nickname="プレイヤー2"
        )
        ...
        self.participant2 = SessionParticipant.objects.create(
            session=self.session, user=self.player2, role="player", character_name="探索者2"
        )
...
class HandoutNotificationServiceTest(TestCase):
    ...
    def test_send_handout_created_notification(self):
        ...
        notifications = HandoutNotification.objects.filter(handout_id=handout.id, recipient=self.player1)
    ...
    def test_send_handout_published_notification(self):
        ...
        notifications = HandoutNotification.objects.filter(handout_id=handout.id, notification_type="handout_published")
        self.assertEqual(notifications.count(), 1)
    ...
    def test_notification_with_disabled_preferences(self):
        ...
        UserNotificationPreferences.objects.create(user=self.player1, handout_notifications_enabled=False)
...
class HandoutNotificationAPITest(TestCase):
    ...
    def test_notification_preferences_api(self):
        ...
        response = client.patch(
            "/api/schedules/notification-preferences/",
            {
