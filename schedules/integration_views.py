...
def _visible_user_sessions(user):
    return TRPGSession.objects.filter(
        Q(gm=user) | Q(sessionparticipant__user=user)
    ).distinct()
...
def _build_ical(user):
    ...
    sessions = _visible_user_sessions(user).filter(
        Q(date__range=(now, end)) | Q(date__isnull=True)
    ).select_related('gm', 'group').order_by('date', 'id')
    lines = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
...
class CalendarSubscriptionView(APIView):
    ...
    def get(self, request, token):
        ...
        subscription = CalendarSubscription.objects.select_related('user').filter(
            token_digest=digest
        ).first()
        if not subscription:
            raise Http404
...
class GoogleCalendarSyncView(APIView):
    ...
    def post(self, request, session_id):
        ...
        if not integration or not integration.has_scope(
...
class GoogleSheetsImportView(APIView):
    ...
    def post(self, request):
        integration = GoogleIntegration.objects.filter(
            user=request.user, sheets_enabled=True
        ).first()
        if not integration or not integration.has_scope(
        ...
        results = [
            {'row': index + 2, 'data': row, 'errors': _validate_character_row(row)}
        ...
        for item in results:
            ...
            if conflict_action == 'update' and row.get('id'):
                instance = CharacterSheet.objects.filter(
                    pk=row['id'], user=request.user
                ).first()
            values = {
                'name': row['name'],
                'edition': row['edition'],
