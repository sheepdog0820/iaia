...
class HandoutManagementDetailTestCase(APITestCase):
    ...
    def test_handout_bulk_creation_with_invalid_data(self):
        ...
        handouts_data = [
            {"participant": self.participant1.id, "title": "Valid Handout", "content": "Valid content"},
            {
                # participant が不足
    ...
    def test_handout_creation_from_template_permission_denied(self):
        ...
        response = self.client.post(
            "/api/schedules/handout-templates/",
            {"template_id": "basic_intro", "session_id": self.session.id, "participant_id": self.participant1.id},
        )
    ...
    def test_handout_queryset_filtering(self):
        ...
        other_session = TRPGSession.objects.create(
            title="Other Session", date=timezone.now() + timedelta(days=2), gm=self.other_user, group=self.group
        )
        ...
        HandoutInfo.objects.create(
            session=other_session, participant=other_participant, title="Other Handout", content="Other content"
        )
