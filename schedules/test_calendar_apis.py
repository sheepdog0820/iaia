...
class MonthlyEventListViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.group = Group.objects.create(
            name="Test Group", description="Test group for calendar API", created_by=self.gm_user
        )
...
class SessionAggregationViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        # グループ作成
        self.group1 = Group.objects.create(name="Group 1", created_by=self.user)
        self.group2 = Group.objects.create(name="Group 2", created_by=self.user)
    ...
    def test_session_aggregation_success(self):
        ...
        gm_sessions = [
            ...
            if s.gm == self.user and s.status in ["planned", "ongoing"] and s.date >= now and s.date <= end_date
        ]
        player_sessions = [
            s
            for s in self.sessions
