...
class SessionParticipant(models.Model):
    ...
    player_slot = models.IntegerField(
        choices=PLAYER_SLOT_CHOICES, null=True, blank=True, help_text="プレイヤー枠番号（1-4）"
    )
...
class SessionInvitation(models.Model):
    ...
    def expire_pending(cls):
        ...
        return cls.objects.filter(status="pending", created_at__lt=cutoff).update(
            status="expired", responded_at=timezone.now()
        )
...
class HandoutInfo(models.Model):
    ...
    recommended_skills = models.TextField(blank=True, default="", help_text="推奨技能（カンマ/改行区切り）")
    is_secret = models.BooleanField(default=True, help_text="秘匿ハンドアウトかどうか")

    # 新規フィールド: ハンドアウト番号
    handout_number = models.IntegerField(null=True, blank=True, help_text="ハンドアウト番号（HO1-HO4）")

    # 新規フィールド: 割り当てプレイヤー枠
    assigned_player_slot = models.IntegerField(
        choices=SessionParticipant.PLAYER_SLOT_CHOICES, null=True, blank=True, help_text="割り当てられたプレイヤー枠"
    )
...
class HandoutView(models.Model):
    handout = models.ForeignKey(HandoutInfo, on_delete=models.CASCADE, related_name="views")
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="viewed_handouts")
...
class SessionImage(models.Model):
    ...
    session = models.ForeignKey(TRPGSession, on_delete=models.CASCADE, related_name="images")
    image = models.ImageField(upload_to="session_images/%Y/%m/%d/", help_text="セッションに関連する画像")
    title = models.CharField(max_length=200, blank=True, help_text="画像のタイトル")
    description = models.TextField(blank=True, help_text="画像の説明")
    order = models.PositiveIntegerField(default=0, help_text="表示順序")
    uploaded_by = models.ForeignKey(
        CustomUser, on_delete=models.SET_NULL, null=True, related_name="uploaded_session_images"
    )
    created_at = models.DateTimeField(default=timezone.now)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and self.order == 0 and hasattr(self, "session_id") and self.session_id:
            max_order = SessionImage.objects.filter(session_id=self.session_id).aggregate(
    ...
    def clean(self):
        ...
        if self.content_type and self.file_type in self.SUPPORTED_CONTENT_TYPES:
            ...
            if self.content_type not in supported_types:
                raise ValidationError(f"サポートされていないファイル形式です: {self.content_type}")
...
class SessionYouTubeLink(models.Model):
    ...
    # リレーション
    session = models.ForeignKey(TRPGSession, on_delete=models.CASCADE, related_name="youtube_links")
    added_by = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, null=True, related_name="added_youtube_links")
    ...
    perspective = models.CharField(
        max_length=100, blank=True, verbose_name="視点", help_text="例: GM視点 / PL1視点 / 全体"
    )
    part_number = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="パート番号", help_text="分割されている場合のパート番号（例: 1,2,3）"
    )
    description = models.TextField(blank=True, verbose_name="備考", help_text="この動画についての説明やメモ")
    order = models.PositiveIntegerField(default=0)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and self.order == 0:
            max_order = SessionYouTubeLink.objects.filter(session_id=self.session_id).aggregate(
                max_order=models.Max("order")
            )["max_order"]
            self.order = (max_order or 0) + 1
    ...
    def get_session_statistics(cls, session):
        ...
        stats = videos.aggregate(
            count=Count("id"), total_duration=Sum("duration_seconds"), avg_duration=Avg("duration_seconds")
        )

        # チャンネル別集計
        channel_stats = (
            videos.values("channel_name")
            .annotate(video_count=Count("id"), total_duration=Sum("duration_seconds"))
...
class SessionSeries(models.Model):
    ...
    group = models.ForeignKey(Group, on_delete=models.CASCADE, related_name="session_series")
    gm = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="gm_session_series")
    scenario = models.ForeignKey(
        "scenarios.Scenario", on_delete=models.SET_NULL, null=True, blank=True, related_name="session_series"
    )

    # 定期開催設定
    recurrence = models.CharField(
        max_length=10, choices=RECURRENCE_CHOICES, default="none", help_text="繰り返しパターン"
    )
    weekday = models.IntegerField(choices=WEEKDAY_CHOICES, null=True, blank=True, help_text="曜日（毎週/隔週の場合）")
    day_of_month = models.IntegerField(null=True, blank=True, help_text="日（毎月の場合、1-31）")
    start_time = models.TimeField(null=True, blank=True, help_text="開始時刻")
    duration_minutes = models.PositiveIntegerField(default=180, help_text="予定時間（分）")

    # カスタム間隔
    custom_interval_days = models.PositiveIntegerField(null=True, blank=True, help_text="カスタム間隔（日数）")

    # 期間
    start_date = models.DateField(help_text="シリーズ開始日")
    end_date = models.DateField(null=True, blank=True, help_text="シリーズ終了日（空の場合は無期限）")
    ...
    def create_session_for_date(self, session_date):
        ...
        if self.start_time:
            ...
            session_datetime = timezone.make_aware(
                datetime.combine(session_date, time_cls(19, 0)), timezone.get_current_timezone()
            )
...
class SessionAvailability(models.Model):
    ...
    proposed_date = models.DateTimeField(
        null=True, blank=True, help_text="候補日時（セッション/オカレンスが未確定の場合）"
    )

    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="availability_votes")
    status = models.CharField(max_length=15, choices=AVAILABILITY_CHOICES, default="available")
    comment = models.CharField(max_length=200, blank=True, help_text="コメント（遅刻/早退など）")
...
class DatePoll(models.Model):
    ...
    group = models.ForeignKey(Group, on_delete=models.CASCADE, related_name="date_polls")
    created_by = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="created_date_polls")

    deadline = models.DateTimeField(null=True, blank=True, help_text="投票締め切り")
    is_closed = models.BooleanField(default=False, help_text="投票が締め切られたか")
    selected_date = models.DateTimeField(null=True, blank=True, help_text="確定日時")
    ...
    def get_vote_summary(self):
        ...
        options = self.options.annotate(
            available_count=Count(Case(When(votes__status="available", then=1), output_field=IntegerField())),
            maybe_count=Count(Case(When(votes__status="maybe", then=1), output_field=IntegerField())),
            unavailable_count=Count(Case(When(votes__status="unavailable", then=1), output_field=IntegerField())),
...
class DatePollOption(models.Model):
    ...
    note = models.CharField(max_length=100, blank=True, help_text="備考")
...
class DatePollVote(models.Model):
    ...
    option = models.ForeignKey(DatePollOption, on_delete=models.CASCADE, related_name="votes")
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="date_poll_votes")
    status = models.CharField(max_length=15, choices=VOTE_CHOICES, default="available")
    comment = models.CharField(max_length=100, blank=True, help_text="コメント")
