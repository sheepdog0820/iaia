...
def walk_conditions(node):
    ...
    if operator:
        ...
        if operator not in {"all", "any"} or not isinstance(conditions, list) or not conditions:
            raise ValidationError("Condition groups require all/any and a non-empty conditions list.")
    ...
    for leaf in walk_conditions(conditions):
        ...
        if condition_type == "handout_viewed":
            try:
                ...
                raise ValidationError("Referenced handout does not exist in this session.") from exc
    ...
    while pending:
        ...
        for leaf in walk_conditions(dependency.release_conditions):
            if leaf["type"] == "handout_viewed":
                pending.extend(HandoutInfo.objects.filter(pk=leaf["value"], session=session))
...
def evaluate_release_conditions(handout, now=None):
    ...
    def evaluate(node):
        ...
        if condition_type == "handout_viewed":
            return (
                bool(handout.participant.user_id)
                and HandoutView.objects.filter(
                    handout_id=value,
