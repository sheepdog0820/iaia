...
class BasicScheduleTestCase(TestCase):
    ...
    def test_session_str_representation(self):
        ...
        session = TRPGSession.objects.create(title="Test Session", date=test_date, gm=self.user1, group=self.group)
    ...
    def test_session_coc_edition_default(self):
        ...
        session = TRPGSession.objects.create(
            title="Edition Default Session", date=timezone.now() + timedelta(days=1), gm=self.user1, group=self.group
        )
