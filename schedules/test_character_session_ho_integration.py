...
class CharacterSessionHOIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        # グループ作成
        self.group = Group.objects.create(name="クトゥルフ神話TRPGグループ", created_by=self.gm, visibility="private")
    ...
    def test_full_integration_flow(self):
        ...
        for i, player in enumerate(self.players):
            ...
            response = self.client.post(reverse("character-sheet-list"), character_data, format="json")
        ...
        for i, (player, character) in enumerate(zip(self.players, characters)):
            assign_data = {"player_slot": i + 1, "user_id": player.id, "character_sheet_id": character.id}  # 1-4の枠

            response = self.client.post(
                reverse("session-assign-player", kwargs={"pk": session.id}), assign_data, format="json"
            )
        ...
        handout_contents = [
            ...
            {"title": "HO3: 古い地図", "content": "あなたの祖父が残した古い地図に、港の沖合に謎の印がつけられている。"},
            {
                "title": "HO4: 調査依頼",
        ...
        handouts = []
        for i in range(4):
            participant = SessionParticipant.objects.get(session=session, player_slot=i + 1)
            ...
            response = self.client.post(reverse("handout-list"), handout_data, format="json")
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        for i, player in enumerate(self.players):
            ...
            # 自分のHOのみが見えることを確認
            my_handouts = [h for h in handout_list if h["session"] == session.id and h["is_secret"]]
            ...
            participant = SessionParticipant.objects.get(session=session, user=player)
    ...
    def test_player_cannot_see_other_handouts(self):
        ...
        session = TRPGSession.objects.create(
            title="テストセッション", date=timezone.now() + timedelta(days=1), gm=self.gm, group=self.group
        )
