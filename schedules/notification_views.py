...
from .serializers import HandoutNotificationSerializer, UserNotificationPreferencesSerializer
...
class HandoutNotificationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        """ユーザーの通知のみを取得"""
        return HandoutNotification.objects.filter(recipient=self.request.user).order_by("-created_at")
    ...
    def _visible_notifications(self, queryset):
        return [notification for notification in queryset if self._can_show_notification(notification)]
    ...
    def mark_read(self, request, pk=None):
        ...
        return Response({"status": "success", "message": "通知を既読にマークしました"})
...
class UserNotificationPreferencesViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        """現在のユーザーの通知設定を取得"""
        return UserNotificationPreferences.get_or_create_for_user(self.request.user)
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response({"status": "success", "message": "通知設定を更新しました", "data": serializer.data})
...
class HandoutNotificationAPIView:
    ...
    def send_test_notification(request):
        ...
        return Response({"status": "success", "message": "テスト通知を送信しました", "notification": serializer.data})
