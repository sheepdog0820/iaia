...
class PlayerSlotHandoutTestCase(APITestCase):
    ...
    def test_assign_player_to_slot(self):
        ...
        data = {"player_slot": 1, "user_id": self.players[0].id, "character_sheet_id": self.characters[0].id}
    ...
    def test_player_join_with_slot(self):
        ...
        data = {"player_slot": 2, "character_sheet_id": self.characters[0].id}
    ...
    def test_create_handouts_with_numbers(self):
        ...
        handouts = []
        for i in range(4):
            participant = SessionParticipant.objects.get(session=self.session, player_slot=i + 1)
        ...
        for i in range(4):
            ...
            visible_handouts = [h for h in handout_list if h["session"] == self.session.id and h["is_secret"]]
    ...
    def test_handout_visibility_by_slot(self):
        ...
        visible_handouts = [h for h in handout_list if h["id"] == handout.id]
        self.assertEqual(len(visible_handouts), 1)
    ...
    def test_gm_can_see_all_handouts(self):
        ...
        session_handouts = [h for h in handout_list if h["session"] == self.session.id]
...
if __name__ == "__main__":
    ...
    failures = test_runner.run_tests(["schedules.test_player_slots_handouts.PlayerSlotHandoutTestCase"])
