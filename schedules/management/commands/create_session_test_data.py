...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        gm1, _ = User.objects.get_or_create(
            username="keeper1", defaults={"email": "keeper1@example.com", "nickname": "深淵の守護者", "is_staff": True}
        )
        ...
        gm2, _ = User.objects.get_or_create(
            username="keeper2", defaults={"email": "keeper2@example.com", "nickname": "古き印の管理人"}
        )
        ...
        for username, nickname in player_names:
            player, _ = User.objects.get_or_create(
                username=username, defaults={"email": f"{username}@example.com", "nickname": nickname}
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"{SessionYouTubeLink.objects.count()}個のYouTube動画リンクを作成しました。")
        )
        ...
        for i, img_data in enumerate(session1_images):
            ...
            image.image.save(img_data["filename"], create_dummy_image(img_data["filename"], img_data["color"]))
            image.save()
