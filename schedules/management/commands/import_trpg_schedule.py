...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        import_sessions = [
            item for item in payload.get("sessions", []) if options.get("include_future") or item.get("is_past", True)
        ]
    ...
    def _extract_excel(self, path: Path, today: date) -> dict:
        ...
        try:
            ...
            raise CommandError(
                "Excel解析には openpyxl が必要です。AWS/本番投入時は --input-json を使用してください。"
            ) from exc
    ...
    def _import_payload(self, payload: dict, group_name: str, default_gm_username: str | None) -> dict:
        ...
        for item in payload.get("sessions", []):
            ...
            visibility = (
                item.get("visibility") if item.get("visibility") in {"private", "group", "link", "public"} else "group"
            )
    ...
    def _ensure_participants(
        ...
        for member in members:
            if not member or member == "未定" or member == (gm.nickname or gm.username):
                continue
    ...
    def _character_name_for_member(self, member: str, members: list[str], character_names: list[str]) -> str:
        ...
        matches = [
            name for name in character_names if self._normalize_participant_name(name).startswith(normalized_member)
        ]
        if matches:
            return " / ".join(matches)
        return ""
