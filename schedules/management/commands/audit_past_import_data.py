...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for scenario in scenarios.exclude(created_by__username=expected_username):
            issues.append(f'scenario creator mismatch: id={scenario.id} title={scenario.title} created_by={scenario.created_by.username}')
        ...
        for scenario in missing_sales:
            ...
            issues.append(f'scenario sales info incomplete: id={scenario.id} title={scenario.title} missing={",".join(missing)}')
        ...
        for session in sessions:
            ...
            if not gm_participant:
                issues.append(f'gm participant missing: session_id={session.id} title={session.title} gm={session.gm.username}')
