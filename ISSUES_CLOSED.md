...
### ✅ ISSUE-006: グループ統計APIの詳細実装（解決済み）
- **カテゴリ**: 機能強化
- **影響範囲**: グループ管理機能
- **詳細**:
  - `/api/accounts/statistics/groups/` - グループ統計API
- **該当ファイル**:
  - `accounts/statistics_views.py` - GroupStatisticsView
  - `accounts/test_statistics.py` - テスト追加
- **実装内容**:
  - ✅ **基本グループ統計** - 実装済み
  - ✅ **グループ活動サマリー（合計/平均/アクティブ/Top GM）** - 実装済み
  - ✅ **メンバー別貢献度（GM/PL回数・プレイ時間）** - 実装済み
  - ✅ **メンバー参加率（%の算出・表示）** - 実装済み
  - ✅ **人気シナリオランキング（グループ内）** - 実装済み
- **解決日**: 2026年1月25日

### ✅ ISSUE-007: ユーザーランキングAPIの詳細実装（解決済み）
- **カテゴリ**: 機能強化
- **影響範囲**: 統計機能
- **詳細**:
  - `/api/accounts/statistics/ranking/` - ユーザーランキングAPI
- **該当ファイル**:
  - `accounts/statistics_views.py` - UserRankingView
  - `accounts/test_statistics.py` - テスト追加
- **実装内容**:
  - ✅ **基本ランキング機能（hours/sessions/gm + year指定）** - 実装済み
  - ✅ **期間別ランキング（month/all/range）** - 実装済み（`period` / `start_date` / `end_date`）
  - ✅ **カテゴリ別ランキング（システム/シナリオ等）** - 実装済み（`category`）
  - ✅ **ランキング推移（時系列）** - 実装済み（`trends` / `trend_unit`）
- **解決日**: 2026年1月25日
...
- **解決日**: 2026年01月15日
- **追補**: co-GM（参加者 role='gm'）もGM同等にノート/ログUI操作可能に改善（2026年1月25日）
...
### ✅ ISSUE-018: セッションYouTubeリンク機能（複数リンク + 並び替えUI）（解決済み）
- **カテゴリ**: 新機能追加
- **影響範囲**: セッション管理
- **該当ファイル**:
  - `templates/schedules/session_detail.html`
  ...
  - `schedules/test_youtube_links.py`
  - `tests/e2e/flows/youtube-links.spec.ts`
- **解決内容**:
  - セッションごとに複数のYouTubeリンクを管理（一覧/追加/編集/削除/一括追加）
  - ドラッグ&ドロップによる並び替え（GMのみ）と順序の永続化
- **テスト**:
  - ✅ `python -m pytest -q schedules`
  - ✅ Playwright（Chromium）`tests/e2e/flows/youtube-links.spec.ts`
- **解決日**: 2026年1月24日

### ✅ ISSUE-043: 通知一覧/設定UI（専用UI + ナビ未読バッジ）（解決済み）
- **カテゴリ**: UI/UX改善
- **影響範囲**: 通知/ナビゲーション
- **詳細**:
...
- **該当ファイル**:
  - `templates/schedules/notifications.html`
  - `templates/base.html`
  - `schedules/urls.py`
  - `templates/account/dashboard.html`
  - `schedules/serializers.py`
  - `schedules/test_notifications_ui.py`
- **解決内容**:
  - 通知一覧ページ（未読/全件、ページング、単体/一括既読）を追加
  - 通知設定（各種ON/OFF）をUIから更新可能に
  - ナビ未読バッジを追加し、定期ポーリング無しで通信回数を最小化（表示時/タブ復帰時 + キャッシュ）
  - 通知から関連セッション詳細へ遷移できるよう改善（handout通知含む）
- **関連コミット**: `d1451f7`
- **解決日**: 2026年1月24日
...
### ✅ ISSUE-019: セッション分析ダッシュボード（解決済み）
- **カテゴリ**: 新機能実装
- **影響範囲**: 統計・分析機能
- **詳細**:
  - セッションデータの統計分析と可視化（Occurrenceベース）
- **UI**:
  - `/api/schedules/analytics/view/`
- **API**:
  - `GET /api/schedules/analytics/dashboard/`（params: `group_id`, `start_date`, `end_date`）
- **該当ファイル**:
  - `schedules/analytics_views.py`
  - `schedules/urls.py`
  - `templates/schedules/analytics_dashboard.html`
  - `templates/base.html`
  - `schedules/test_analytics_dashboard.py`
- **実装内容**:
  - ✅ **参加率分析** - 実装済み（グループ選択時に平均参加率を算出）
  - ✅ **人気時間帯分析** - 実装済み
  - ✅ **セッション推移（月別）** - 実装済み
  - ✅ **GM負荷分析** - 実装済み
  - ✅ **プレイヤー相性（共演回数Top）** - 実装済み
- **解決日**: 2026年1月25日

### ✅ ISSUE-042: セッションテンプレート（解決済み）
- **カテゴリ**: 新機能実装
- **影響範囲**: セッション作成
- **詳細**:
...
- **API**:
  - `/api/schedules/session-templates/` - テンプレートCRUD
- **該当ファイル**:
  - `schedules/models.py`
  - `schedules/migrations/0028_sessiontemplate.py`
  - `schedules/serializers.py`
  - `schedules/template_views.py`
  - `schedules/urls.py`
  - `schedules/admin.py`
  - `templates/schedules/session_templates.html`
  - `templates/schedules/calendar.html`
  - `schedules/test_session_templates.py`
- **実装内容**:
  - ✅ **テンプレートCRUD** - 実装済み
  - ✅ **テンプレの適用/保存（カレンダーの新規作成モーダル）** - 実装済み
  - ✅ **カレンダー入力からの即時登録・詳細編集への引継ぎ** - 実装済み
  - ✅ **テンプレート画像・秘匿ハンドアウト雛形の管理とセッションへの複製** - 実装済み
- **解決日**: 2026年1月25日

### ✅ ISSUE-041: 経験値配布（解決済み）
- **カテゴリ**: 新機能実装
- **影響範囲**: セッション管理 / キャラクター管理
- **詳細**:
  - GM/co-GMが参加者の経験値・特別報酬・備考を記録し、必要なら成長記録へ反映できるようにする
- **UI**:
  - `/api/schedules/sessions/{id}/detail/`（経験値配布カード）
- **API**:
  - `GET/POST /api/schedules/sessions/{session_id}/rewards/`
  - `PATCH /api/schedules/rewards/{id}/`
  - `POST /api/schedules/rewards/{id}/apply/`（成長記録へ反映）
- **該当ファイル**:
  - `schedules/models.py`
  - `schedules/migrations/0029_sessionreward.py`
  - `schedules/serializers.py`
  - `schedules/reward_views.py`
  - `schedules/urls.py`
  - `templates/schedules/session_detail.html`
  - `schedules/test_session_rewards.py`
- **実装内容**:
  - ✅ **配布内容の記録モデル** - 実装済み（SessionReward）
  - ✅ **GMによる入力UI** - 実装済み（セッション詳細）
  - ✅ **キャラクター成長記録への反映（任意）** - 実装済み（apply）
- **解決日**: 2026年1月25日

### ✅ ISSUE-023: セッション準備・事後処理ツール（Epic）（解決済み）
- **カテゴリ**: 新機能実装（Epic）
- **影響範囲**: セッション管理機能
- **対応結果**:
  - 🚫 ISSUE-039: セッション準備チェックリスト（MVP）（取り下げ）
  - 🚫 ISSUE-040: セッション後フィードバック（MVP）（取り下げ）
  - ✅ ISSUE-041: 経験値配布（後続）（実装済み）
  - ✅ ISSUE-042: セッションテンプレート（後続）（実装済み）
- **解決日**: 2026年1月25日
 
### ✅ ISSUE-049: プレミアムユーザー権限と画面閲覧制限（解決済み）
- **カテゴリ**: 新機能実装
- **影響範囲**: 認可/画面表示
- **詳細**:
  - 高権限（課金）ユーザーを手動で設定できる
  - シナリオ関連のAPI結果確認画面（アーカイブ画面）をプレミアムユーザーのみ閲覧可
- **該当ファイル**:
  - `accounts/models/user_models.py`
  - `accounts/migrations/0030_customuser_is_premium.py`
  - `accounts/admin.py`
  - `accounts/management/commands/set_premium_user.py`
  - `scenarios/views.py`
  - `scenarios/urls.py`
  - `templates/accounts/premium_required.html`
  - `templates/base.html`
  - `templates/home.html`
  - `scenarios/test_scenarios.py`
  - `tests/ui/test_ui_navigation.py`
- **実装内容**:
  - ✅ **プレミアムフラグ（is_premium）追加** - 実装済み
  - ✅ **管理画面での切り替え** - 実装済み
  - ✅ **管理コマンドでの切り替え** - 実装済み
  - ✅ **シナリオアーカイブ画面の閲覧制限** - 実装済み（非対象は403 + 案内ページ）
  - ✅ **ナビ/導線の表示制御** - 実装済み
- **備考**: 決済/課金導線は未実装（手動設定で運用）
- **解決日**: 2026年1月25日
...
- **影響範囲**: セッション管理（事後処理）
- **詳細**:
  - 参加者の感想/評価収集は当面不要のため、実装対象から除外
- **解決日**: 2026年1月25日

### 🚫 ISSUE-046: セッション側キャラクター状態のスナップショット/差分履歴（取り下げ）
- **カテゴリ**: 機能強化（取り下げ）
- **影響範囲**: セッション管理 / キャラクター管理
- **詳細**:
  - 当面不要のため、実装対象から除外
