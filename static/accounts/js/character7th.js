document.addEventListener('DOMContentLoaded', function() {
    ...
    const SKILL_CATEGORY_KEYS = {
        combat: new Set(Object.keys(SKILLS_7TH.combat)),
        exploration: new Set(Object.keys(SKILLS_7TH.exploration)),
        action: new Set(Object.keys(SKILLS_7TH.action)),
    ...
    const ALL_SKILLS_7TH = {
        ...SKILLS_7TH.combat,
        ...SKILLS_7TH.exploration,
        ...SKILLS_7TH.action,
        ...SKILLS_7TH.social,
        ...SKILLS_7TH.knowledge
    };
    ...
    const DEFAULT_ABILITY_DICE_SETTINGS_7TH = {
        str: { count: 3, sides: 6, bonus: 0, multiplier: 5 },
        con: { count: 3, sides: 6, bonus: 0, multiplier: 5 },
        pow: { count: 3, sides: 6, bonus: 0, multiplier: 5 },
        dex: { count: 3, sides: 6, bonus: 0, multiplier: 5 },
        app: { count: 3, sides: 6, bonus: 0, multiplier: 5 },
        siz: { count: 2, sides: 6, bonus: 6, multiplier: 5 },
        int: { count: 2, sides: 6, bonus: 6, multiplier: 5 },
        edu: { count: 2, sides: 6, bonus: 6, multiplier: 5 }
    };
...
function updateGlobalDiceFormula() {
        ...
        const multiplier = parseInt(document.getElementById('globalDiceMultiplier')?.value) || 5;
        
        const formula = `${count}d${sides}${bonus >= 0 ? '+' : ''}${bonus}${multiplier === 1 ? '' : `×${multiplier}`}`;
        const formulaSpan = document.getElementById('globalDiceFormula');
        if (formulaSpan) {
            formulaSpan.textContent = formula;
        }
        
        return { count, sides, bonus, multiplier };
    }

    function formatDiceFormula(count, sides, bonus, multiplier = 1) {
        const sign = bonus >= 0 ? '+' : '';
        const mult = multiplier === 1 ? '' : `×${multiplier}`;
        return `${count}d${sides}${sign}${bonus}${mult}`;
    }

    function parseDiceFormula(formula) {
        if (!formula) return null;
        const match = formula.trim().match(/^(\d+)\s*[dD]\s*(\d+)\s*([+-]\s*\d+)?\s*(?:[x×]\s*(\d+))?$/);
        if (!match) return null;
        const count = parseInt(match[1], 10);
        const sides = parseInt(match[2], 10);
        const bonus = match[3] ? parseInt(match[3].replace(/\s+/g, ''), 10) : 0;
        const multiplier = match[4] ? parseInt(match[4], 10) : 1;
        if (!Number.isFinite(count) || !Number.isFinite(sides) || !Number.isFinite(bonus) || !Number.isFinite(multiplier)) return null;
        if (count < 1 || count > 10) return null;
        if (sides < 2 || sides > 100) return null;
        if (bonus < -50 || bonus > 50) return null;
        if (multiplier < 1 || multiplier > 20) return null;
        return { count, sides, bonus, multiplier };
    }
    ...
    function setAbilityDiceSettingsToDefault7th() {
        abilityDiceSettings = Object.fromEntries(
            ABILITIES_6TH.map(ability => [
                ability,
                { ...(DEFAULT_ABILITY_DICE_SETTINGS_7TH[ability] ?? updateGlobalDiceFormula()) }
            ])
        );
    }
    ...
    function buildAbilityDiceSettingsHTML() {
        ...
        if (!Object.keys(abilityDiceSettings).length) {
            setAbilityDiceSettingsToDefault7th();
        }
                ...
                <div class="col-md-3 mb-3">
                    <div class="dice-setting-item border rounded p-2 h-100">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <input type="text" class="form-control" id="ability-dice-formula-${ability}" value="${formatDiceFormula(settings.count, settings.sides, settings.bonus, settings.multiplier)}" placeholder="3d6+0×5">
                        </div>
                    </div>
                </div>
            `;
    ...
    function attachAbilityDiceSettingListeners() {
        ABILITIES_6TH.forEach(ability => {
            ...
            const syncFormula = () => {
                ...
                const multiplier = abilityDiceSettings[ability]?.multiplier ?? 1;
                abilityDiceSettings[ability] = { count, sides, bonus, multiplier };
                if (formulaInput) {
                    formulaInput.value = formatDiceFormula(count, sides, bonus, multiplier);
    ...
    function rollSingleAbility(ability) {
        ...
        const result = rollDice(settings.count, settings.sides, settings.bonus, settings.multiplier);
    ...
    // ダイスロール関数
    function rollDice(count, sides, bonus, multiplier = 1) {
        let total = 0;
    ...
    // Derived stats specific to 7th edition
    ...
    function getSkillDisplayName(key, card) {
        ...
        return ALL_SKILLS_7TH[key]?.name || key;
    }
        ...
        card.querySelectorAll('.skill-base').forEach(input => {
            ...
            input.addEventListener('contextmenu', function(e) {
                ...
                try {
                    ...
                    const skill = ALL_SKILLS_7TH[skillKey];
                    if (!skill) {
                        return;
                    }
    ...
    function generateSkillsList() {
        ...
        Object.entries(SKILLS_7TH).forEach(([category, skills]) => {
    ...
    function renderRecommendedChips() {
        ...
        Array.from(recommendedSkillKeys).forEach(key => {
            const name = ALL_SKILLS_7TH[key]?.name || key;
    ...
    function updateDynamicSkillBases() {
        try {
            ...
            if (dodgeBaseEl && (!window.customBaseValues || window.customBaseValues['dodge'] === undefined)) {
                dodgeBaseEl.value = Math.min(Math.floor(dex / 2), 999);
            }
            ...
            if (languageOwnBaseEl && (!window.customBaseValues || window.customBaseValues['language_own'] === undefined)) {
                languageOwnBaseEl.value = Math.min(edu, 999);
            }
...
function initOccupationTemplates() {
    ...
    function displayOccupations(category) {
        ...
        occupations.forEach(occ => {
            const skillDisplay = (occ.skills || [])
                .map(skillKey => ALL_SKILLS_7TH[skillKey]?.name || skillKey)
                .join(', ');
            html += `
    ...
    async function loadCharacterForEdit(characterId) {
        ...
        if (titleEl) {
            titleEl.innerHTML = '<i class="fas fa-user-ninja text-primary"></i> クトゥルフ神話TRPG 7版探索者編集';
        }
        document.title = 'クトゥルフ神話TRPG 7版探索者編集 - タブレノ';
        ...
        if (sheet.edition !== '7th') {
            throw { error: 'この画面は7版キャラクター専用です。' };
        }
        ...
        (sheet.skills || []).forEach(skill => {
            ...
            const defaultBase = (() => {
                const definitionBase = ALL_SKILLS_7TH[skillKey]?.base;
                if (typeof definitionBase === 'number') return definitionBase;
                if (definitionBase === 'DEX/2') {
                    const dex = parseInt(document.getElementById('dex')?.value) || 0;
                    return Math.min(Math.floor(dex / 2), 999);
                }
                if (definitionBase === 'EDU') {
                    const edu = parseInt(document.getElementById('edu')?.value) || 0;
                    return Math.min(edu, 999);
                }
                return 0;
            })();
