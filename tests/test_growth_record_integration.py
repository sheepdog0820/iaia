...
from accounts.character_models import CharacterSheet, CharacterSheet6th, CharacterSkill, GrowthRecord, SkillGrowthRecord
...
class GrowthRecordIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        # 6版データ作成
        CharacterSheet6th.objects.create(character_sheet=self.character, mental_disorder="")
    ...
    def test_create_growth_record(self):
        """成長記録の作成テスト"""
        url = reverse("growth-records-list", kwargs={"character_sheet_id": self.character.id})
        ...
        # データベースに保存されたか確認
        growth_record = GrowthRecord.objects.get(character_sheet=self.character, scenario_name="クトゥルフの呼び声")
    ...
    def test_create_growth_record_with_skill_growths(self):
        ...
        # スキル成長が保存されたか確認
        growth_record = GrowthRecord.objects.get(character_sheet=self.character, scenario_name="ダンウィッチの怪")
    ...
    def test_add_skill_growth_to_existing_record(self):
        ...
        url = reverse(
            "growth-records-add-skill", kwargs={"character_sheet_id": self.character.id, "pk": growth_record.id}
        )
        ...
        # スキル成長が追加されたか確認
        skill_growth = SkillGrowthRecord.objects.get(growth_record=growth_record, skill_name="回避")
    ...
    def test_growth_summary(self):
        ...
        url = reverse("growth-records-summary", kwargs={"character_sheet_id": self.character.id})
        response = self.client.get(url)
    ...
    def test_delete_growth_record(self):
        ...
        url = reverse("growth-records-detail", kwargs={"character_sheet_id": self.character.id, "pk": growth_record.id})

        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

        # 削除されたか確認
        self.assertFalse(GrowthRecord.objects.filter(id=growth_record.id).exists())
    ...
    def test_unauthorized_access(self):
        ...
        # 他人のキャラクターの成長記録にアクセス
        url = reverse("growth-records-list", kwargs={"character_sheet_id": other_character.id})
