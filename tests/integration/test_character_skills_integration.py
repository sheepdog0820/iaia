...
class CharacterSkillsIntegrationTest(TestCase):
    ...
    def setUp(self):
        """テストユーザーとクライアントの準備"""
        self.user = User.objects.create_user(username="test_user", password="testpass123", email="test@example.com")
        self.client = Client()
    ...
    def test_character_creation_with_skills(self):
        ...
        print(
            f"[INFO] 技能データ数: {len([k for k in form_data.keys() if k.startswith('skill_') and k.endswith('_name')])}"
        )
        ...
        print(
            f"  - 能力値: STR={character.str_value}, DEX={character.dex_value}, INT={character.int_value}, EDU={character.edu_value}"
        )
        ...
        for skill_name in expected_skills:
            ...
            print(
                f"    {skill_name}: 基本={skill.base_value}, 職業={skill.occupation_points}, 趣味={skill.interest_points}, 合計={skill.current_value}"
            )
    ...
    def test_skill_calculation_logic(self):
        ...
        for skill in skills:
            ...
            self.assertEqual(skill.current_value, expected_total, f"技能「{skill.skill_name}」の合計値が正しくない")
            print(
                f"  [OK] {skill.skill_name}: {skill.base_value}+{skill.occupation_points}+{skill.interest_points}+{skill.other_points}={skill.current_value}"
            )
    ...
    def test_derived_stats_calculation(self):
        ...
        print(
            f"  [OK] HP: {character.hit_points_current}/{character.hit_points_max} = ceil((CON{character.con_value} + SIZ{character.siz_value}) / 2)"
        )
        ...
        print(
            f"  [OK] 正気度: {character.sanity_current}/{character.sanity_max} (初期:{character.sanity_starting}) = POW{character.pow_value} × 5"
        )
