...
class CustomSkillIntegrationTest(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.selenium.add_cookie({"name": "sessionid", "value": session_cookie.value, "secure": False, "path": "/"})
    ...
    def test_add_custom_skill_to_combat_category(self):
        ...
        WebDriverWait(self.selenium, 10).until(EC.visibility_of_element_located((By.ID, "abilities")))
        abilities = ["str", "con", "pow", "dex", "app", "siz", "int", "edu"]
        for ability in abilities:
            ability_input = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, ability)))
            ability_input.clear()
        ...
        for i in range(3):
            add_button = WebDriverWait(self.selenium, 20).until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "#knowledge button[data-custom-skill-category='knowledge']")
                )
            )
            self.selenium.execute_script("arguments[0].click();", add_button)
            time.sleep(0.5)
    ...
    def test_custom_skill_appears_in_all_skills_tab(self):
        ...
        add_button = WebDriverWait(self.selenium, 20).until(
            EC.element_to_be_clickable(
                (By.CSS_SELECTOR, "#exploration button[data-custom-skill-category='exploration']")
            )
        )
        self.selenium.execute_script("arguments[0].click();", add_button)
        time.sleep(1)
        ...
        WebDriverWait(self.selenium, 20).until(EC.visibility_of_element_located((By.ID, "all")))
