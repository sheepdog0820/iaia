...
class CharacterSessionIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.gm_user = CustomUser.objects.create_user(
            username="gm_master", email="gm@example.com", password="gmpass123", nickname="ゲームマスター"
        )

        self.player_user = CustomUser.objects.create_user(
            username="player1", email="player1@example.com", password="playerpass123", nickname="プレイヤー1"
        )

        self.player2_user = CustomUser.objects.create_user(
            username="player2", email="player2@example.com", password="playerpass123", nickname="プレイヤー2"
        )
    ...
    def test_complete_character_session_workflow(self):
        ...
        for skill in skills_data:
            ...
            if skill_response.status_code != status.HTTP_201_CREATED:
                print(
                    f"Skill creation failed: {skill_response.status_code} - {getattr(skill_response, 'data', skill_response.content)}"
                )
        ...
        equipment_data = [
            ...
            {"name": "懐中電灯", "quantity": 1, "description": "夜間調査用", "item_type": "item"},
            {"name": "ノートと筆記用具", "quantity": 1, "description": "調査記録用", "item_type": "item"},
        ]

        for equip in equipment_data:
            equip_response = self.client.post(
                f"/api/accounts/character-sheets/{character_id}/equipment/",
                {"character_sheet": character_id, **equip},  # item_typeは各装備データに含まれている
                format="json",
            )
            if equip_response.status_code != status.HTTP_201_CREATED:
                print(
                    f"Equipment creation failed: {equip_response.status_code} - {getattr(equip_response, 'data', equip_response.content)}"
                )
        ...
        print(
            f"Participant data: character_name='{participant.character_name}', character_sheet_id={participant.character_sheet_id}"
        )
...
class CharacterReferenceInSessionTestCase(APITestCase):
    ...
    def setUp(self):
        """テストデータのセットアップ"""
        # ユーザーとグループ作成
        self.gm_user = CustomUser.objects.create_user(username="gm", email="gm@test.com", password="gmpass")
        self.player_user = CustomUser.objects.create_user(
            username="player", email="player@test.com", password="playerpass"
        )
    ...
    def test_session_participant_exposes_character_reference(self):
        ...
        character_name = participant_data.get("character_name") or participant_data.get(
...
class CharacterSessionValidationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.session = TRPGSession.objects.create(
            title="Test Session", date=timezone.now() + timedelta(days=1), gm=self.user, group=self.group
        )
    ...
    def test_cannot_join_session_without_group_membership(self):
        ...
        response = self.client.post(join_url, {"character_name": "Unauthorized Character"})
