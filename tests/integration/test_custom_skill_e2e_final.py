...
class CustomSkillE2EFinalTest(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.keeper1 = User.objects.create_user(username="keeper1", email="keeper1@example.com", password="keeper123")
    ...
    def test_custom_skill_add_and_remove(self):
        ...
        WebDriverWait(self.selenium, 20).until(EC.presence_of_element_located((By.ID, "character-name")))
        ...
        try:
            ...
            # 能力値を入力（能力値タブへ移動してから）
            abilities_tab = WebDriverWait(self.selenium, 20).until(EC.element_to_be_clickable((By.ID, "abilities-tab")))
            self.selenium.execute_script("arguments[0].click();", abilities_tab)
            WebDriverWait(self.selenium, 20).until(EC.visibility_of_element_located((By.ID, "abilities")))
            abilities = ["str", "con", "pow", "dex", "app", "siz", "int", "edu"]
            for ability in abilities:
                element = WebDriverWait(self.selenium, 20).until(EC.element_to_be_clickable((By.ID, ability)))
                element.clear()
        ...
        try:
            ...
            WebDriverWait(self.selenium, 20).until(EC.visibility_of_element_located((By.ID, "skills")))
            print("技能タブに移動しました")
        except Exception as e:
            print(f"技能タブエラー: {e}")
