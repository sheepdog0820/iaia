...
class AuthenticationPermissionIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.group_member = User.objects.create_user(
            username="group_member", email="member@arkham.edu", password="member_2024", nickname="Group Member"
        )

        self.external_user = User.objects.create_user(
            username="external_user", email="external@arkham.edu", password="external_2024", nickname="External User"
        )
        ...
        self.scenario = Scenario.objects.create(
            title="Test Authentication Scenario", author="Test Author", game_system="coc", created_by=self.group_admin
        )
    ...
    def test_unauthenticated_access_restrictions(self):
        ...
        for endpoint in restricted_endpoints:
            ...
            self.assertIn(response.status_code, expected_statuses, f"Endpoint {endpoint} should require authentication")
    ...
    def test_group_visibility_access_control(self):
        ...
        response = self.client.patch(f"/api/accounts/groups/{self.private_group.id}/", {"name": "Modified Name"})
...
class CompleteWorkflowIntegrationTestCase(TransactionTestCase):
    ...
    def test_complete_trpg_circle_lifecycle(self):
        ...
        completed_sessions = TRPGSession.objects.filter(group=circle, status="completed")
        ...
        # 各プレイヤーの統計確認
        for player in active_players:
            player_histories = PlayHistory.objects.filter(user=player, scenario_id=campaign_id)
            self.assertEqual(player_histories.count(), 3)  # 3 sessions

            # 参加セッション時間の計算
            player_sessions = TRPGSession.objects.filter(participants=player, status="completed")
            total_time = sum(session.duration_minutes for session in player_sessions)
            self.assertEqual(total_time, 1080)  # 18 hours total

        # GMの統計確認
        gm_histories = PlayHistory.objects.filter(user=self.gm, scenario_id=campaign_id, role="gm")
...
class APIErrorHandlingIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="test_user", email="test@arkham.edu", password="test_pass_2024", nickname="Test User"
        )
    ...
    def test_api_error_handling_workflow(self):
        ...
        # 2. 不正なデータでのリソース作成
        invalid_group_data = {"name": "", "visibility": "invalid_choice"}  # 必須フィールドが空  # 無効な選択肢
        ...
        other_user = User.objects.create_user(
            username="other_user", email="other@arkham.edu", password="other_pass_2024"
        )
        other_group = CustomGroup.objects.create(name="Other Group", created_by=other_user, visibility="private")

        response = self.client.get(f"/api/accounts/groups/{other_group.id}/")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        # 4. 重複データの作成試行
        group = CustomGroup.objects.create(name="Test Group", created_by=self.user)
        GroupMembership.objects.create(user=self.user, group=group, role="admin")

        # 同じユーザーを再度追加試行
        duplicate_membership_data = {"user": self.user.id, "group": group.id, "role": "member"}
