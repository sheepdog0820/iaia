...
class Character6thFullWorkflowIntegrationTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="player1", password="testpass123", email="player1@example.com")
        self.gm = User.objects.create_user(username="gamemaster", password="gmpass123", email="gm@example.com")
    ...
    def test_complete_character_lifecycle(self):
        ...
        response = self.client.post(
            f"/api/accounts/character-sheets/{character_id}/batch_allocate_skill_points/", skills_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        equipment_list = [
            ...
            {"equipment_type": "armor", "name": "革のジャケット", "armor_value": 1, "weight": "2.0", "quantity": 1},
            {"equipment_type": "item", "name": "懐中電灯", "weight": "0.3", "quantity": 1},
            {"equipment_type": "item", "name": "カメラ", "weight": "0.5", "quantity": 1},
            {"equipment_type": "item", "name": "調査ノート", "weight": "0.2", "quantity": 3},
            {"equipment_type": "item", "name": "応急手当キット", "weight": "1.0", "quantity": 1},
        ]

        for item in equipment_list:
            response = self.client.post(f"/api/accounts/character-sheets/{character_id}/equipment/", item)
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.client.post(f"/api/accounts/character-sheets/{character_id}/background/", background_data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        self.client.force_authenticate(user=self.user)
        response = self.client.post(
            f"/api/schedules/sessions/{session_id}/register/", {"character_sheet_id": character_id}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # Skill improvement
        listen_skill = CharacterSkill.objects.get(character_sheet=character, skill_name="聞き耳")
        ...
        growth_record = GrowthRecord.objects.create(
            ...
            changes={
                ...
                "skills": {"聞き耳": {"before": 70, "after": 73}, "クトゥルフ神話": {"before": 0, "after": 5}},
            },
        )
        ...
        for i in range(2, 5):
            ...
            self.client.force_authenticate(user=self.user)
            response = self.client.post(
                f"/api/schedules/sessions/{session_id}/register/", {"character_sheet_id": version_id}
            )
...
class Character6thConcurrencyIntegrationTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        for i in range(3):
            user = User.objects.create_user(username=f"player{i}", password="testpass123")
    ...
    def test_concurrent_skill_allocation(self):
        ...
        def allocate_skills(client, character_id, skill_name):
            return client.post(
                ...
                {"skill_name": skill_name, "base_value": 5, "occupation_points": 50, "interest_points": 10},
            )
    ...
    def test_concurrent_character_modification(self):
        ...
        def update_character(client, character_id, update_data):
            return client.patch(f"/api/accounts/character-sheets/{character_id}/", update_data)
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            # Owner update
            futures.append(executor.submit(update_character, self.clients[0], character.id, {"age": 26}))

            # Non-owner updates (should fail)
            futures.append(executor.submit(update_character, self.clients[1], character.id, {"age": 27}))
            futures.append(executor.submit(update_character, self.clients[2], character.id, {"age": 28}))
...
class Character6thPermissionIntegrationTest(TransactionTestCase):
    ...
    def test_character_modification_permissions(self):
        ...
        # Owner can modify
        client.force_authenticate(user=self.owner)
        response = client.patch(f"/api/accounts/character-sheets/{self.public_char.id}/", {"age": 26})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # Non-owner cannot modify even public character
        client.force_authenticate(user=self.stranger)
        response = client.patch(f"/api/accounts/character-sheets/{self.public_char.id}/", {"age": 27})
    ...
    def test_skill_allocation_permissions(self):
        ...
        response = client.post(
            ...
            {"skill_name": "医学", "base_value": 5, "occupation_points": 50},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = client.post(
            f"/api/accounts/character-sheets/{self.public_char.id}/allocate_skill_points/",
            {"skill_name": "心理学", "base_value": 5, "occupation_points": 30},
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class Character6thSessionIntegrationTest(TransactionTestCase):
    ...
    def setUp(self):
        self.gm = User.objects.create_user(username="gm", password="gmpass")
        self.player = User.objects.create_user(username="player", password="playerpass")
    ...
    def test_session_participation_with_character(self):
        ...
        response = client.post(
            f"/api/schedules/sessions/{self.session.id}/register/", {"character_sheet_id": self.character.id}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class Character6thDataIntegrityTest(TransactionTestCase):
    ...
    def test_character_deletion_cascades(self):
        ...
        # Add related data
        skill = CharacterSkill.objects.create(character_sheet=character, skill_name="医学", base_value=5)

        equipment = CharacterEquipment.objects.create(
            character_sheet=character, equipment_type="item", name="Test Item"
        )

        background = CharacterBackground.objects.create(character_sheet=character, description="Test background")

        growth = GrowthRecord.objects.create(
            character_sheet=character, session_date="2024-01-01", session_title="Test Session"
        )

        dice_setting = CharacterDiceRollSetting.objects.create(
            character_sheet=character, name="Test Roll", dice_count=2, dice_sides=6
        )
...
class Character6thPerformanceIntegrationTest(TransactionTestCase):
    ...
    def test_large_character_list_performance(self):
        ...
        for i in range(50):
            char = CharacterSheet(
                ...
                int_value=10 + ((i + 6) % 8),
                edu_value=10 + ((i + 7) % 8),
                access_scope="public" if i % 3 == 0 else "private",  # Every 3rd is public
            )
        ...
        response = self.client.get(
            "/api/accounts/character-sheets/", {"is_active": True, "access_scope": "public", "search": "Character 01"}
        )
        elapsed = time.time() - start_time
    ...
    def test_batch_operation_performance(self):
        ...
        start_time = time.time()
        response = self.client.post(
            f"/api/accounts/character-sheets/{character.id}/batch_allocate_skill_points/", skills_data, format="json"
        )
        elapsed = time.time() - start_time

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(CharacterSkill.objects.filter(character_sheet=character).count(), 25)
