...
class UserGroupIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.gm_user = User.objects.create_user(
            username="gm_master", email="gm@arkham.com", password="arkham_gm_2024", nickname="Ancient GM"
        )

        self.player1 = User.objects.create_user(
            username="investigator1", email="player1@arkham.com", password="cthulhu_2024", nickname="Brave Investigator"
        )

        self.player2 = User.objects.create_user(
            username="investigator2", email="player2@arkham.com", password="cthulhu_2024", nickname="Wise Scholar"
        )

        self.outsider = User.objects.create_user(
            username="outsider", email="outsider@arkham.com", password="outsider_2024", nickname="Curious Outsider"
        )
    ...
    def test_group_creation_and_member_management_flow(self):
        ...
        membership_exists = GroupMembership.objects.filter(user=self.player1, group=group, role="member").exists()
    ...
    def test_access_control_by_group_visibility(self):
        ...
        # プライベートグループ作成
        self.client.force_authenticate(user=self.gm_user)
        private_group_data = {"name": "Secret Cult", "description": "Hidden from prying eyes", "visibility": "private"}
...
class SessionScenarioIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        # ユーザーとグループ作成
        self.gm_user = User.objects.create_user(
            username="keeper", email="keeper@arkham.com", password="keeper_2024", nickname="The Keeper"
        )

        self.player = User.objects.create_user(
            username="player", email="player@arkham.com", password="player_2024", nickname="Investigator"
        )

        self.group = CustomGroup.objects.create(
            name="Call of Cthulhu Circle", created_by=self.gm_user, visibility="private"
        )
...
class AuthenticationAndPermissionIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.regular_user = User.objects.create_user(
            username="regular", email="regular@arkham.com", password="regular_2024", nickname="Regular User"
        )

        self.group = CustomGroup.objects.create(name="Test Group", created_by=self.admin_user)
    ...
    def test_resource_ownership_permissions(self):
        ...
        # 自分が作成したグループは編集可能
        own_group = CustomGroup.objects.create(name="My Group", created_by=self.regular_user)
...
class EndToEndBusinessFlowTestCase(TransactionTestCase):
    ...
    def test_complete_trpg_campaign_workflow(self):
        ...
        # フェーズ3: セッション実行（複数回）
        session_dates = [timezone.now() + timedelta(days=i * 7) for i in range(1, 4)]
