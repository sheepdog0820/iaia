...
class PlayerWorkflowIntegrationTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.gm = User.objects.create_user(
            username="test_gm", email="gm@arkham.test", password="test_password_123", nickname="テストGM"
        )

        # 他のプレイヤー作成
        self.other_player = User.objects.create_user(
            username="other_player", email="other@arkham.test", password="test_password_123", nickname="他のプレイヤー"
        )
    ...
    def test_complete_player_workflow(self):
        ...
        group_data = {"name": "テストTRPGサークル", "description": "テスト用のTRPGグループです", "visibility": "public"}
        ...
        # セッション参加
        join_data = {"character_name": "探偵ホームズ", "character_sheet_url": "https://example.com/character/1"}
...
class GMWorkflowIntegrationTest(APITestCase):
    ...
    def test_complete_gm_workflow(self):
        ...
        # 2. グループ作成・管理
        print("\\n2) グループ作成・管理")
        group_data = {"name": "GM主催TRPGサークル", "description": "GM主催のテストグループ", "visibility": "public"}
        response = self.client.post("/api/accounts/groups/", group_data)
        self.assertEqual(response.status_code, 201)
        group_id = response.data["id"]
        print(f"   [OK] グループ作成: {response.data['name']}")
        ...
        # プレイヤーがセッションに参加
        self.client.force_authenticate(user=self.player1)
        join_data = {"character_name": "ジョン・スミス", "character_sheet_url": "https://example.com/character/smith"}
        response = self.client.post(f"/api/schedules/sessions/{session_id}/join/", join_data)
        self.assertEqual(response.status_code, 201)
        ...
        # セッションステータス更新
        session_update_data = {"status": "completed"}
...
class CalendarFilterIntegrationTest(APITestCase):
    ...
    def test_calendar_filter_functionality(self):
        ...
        public_group_data = {"name": "公開グループ", "description": "公開テスト用", "visibility": "public"}
...
class ExportStatisticsIntegrationTest(APITestCase):
    ...
    def create_test_data(self):
        ...
        for session_data in sessions:
            session_data.update({"group": self.group_id, "visibility": "group"})
            response = self.client.post("/api/schedules/sessions/", session_data)
