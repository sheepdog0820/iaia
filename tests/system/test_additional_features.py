...
class ExportFunctionTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="export_user", email="export@arkham.edu", password="export_2024", nickname="Export User"
        )

        # テストデータ作成
        self.group = CustomGroup.objects.create(name="Export Test Group", created_by=self.user)

        GroupMembership.objects.create(user=self.user, group=self.group, role="admin")

        self.scenario = Scenario.objects.create(
            title="Export Test Scenario", author="Test Author", game_system="coc", created_by=self.user
        )
    ...
    def test_export_with_date_range(self):
        ...
        response = self.client.get(
            "/api/accounts/export/statistics/", {"format": "json", "start_date": start_date, "end_date": end_date}
        )
...
class StatisticsFunctionTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="stats_user", email="stats@arkham.edu", password="stats_2024", nickname="Stats User"
        )

        # テストデータ作成
        self.group = CustomGroup.objects.create(name="Stats Test Group", created_by=self.user)
        ...
        for i in range(5):
            scenario = Scenario.objects.create(
                title=f"Stats Test Scenario {i+1}", author="Test Author", game_system="coc", created_by=self.user
            )
...
class HandoutManagementTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.gm = User.objects.create_user(
            username="gm_handout", email="gm@arkham.edu", password="gm_2024", nickname="GM User"
        )

        self.player1 = User.objects.create_user(
            username="player1_handout", email="player1@arkham.edu", password="player_2024", nickname="Player 1"
        )

        self.player2 = User.objects.create_user(
            username="player2_handout", email="player2@arkham.edu", password="player_2024", nickname="Player 2"
        )

        self.group = CustomGroup.objects.create(name="Handout Test Group", created_by=self.gm)
...
class CalendarIntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="calendar_user", email="calendar@arkham.edu", password="calendar_2024", nickname="Calendar User"
        )

        self.group = CustomGroup.objects.create(name="Calendar Test Group", created_by=self.user)
