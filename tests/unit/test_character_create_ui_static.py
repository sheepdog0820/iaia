...
class CharacterCreateUiStaticTests(SimpleTestCase):
    ...
    def test_character_detail_reference_background_is_pdf_backstory_only(self):
        ...
        background_block = self.extract_function_block(
            template,
            'function displayBackgroundInfo(character)',
            '// 装備表示'
        )
    ...
    def test_legacy_character_edit_uses_current_background_fields(self):
        template = self.read_text('templates/accounts/character_edit.html')
        background_block = self.extract_between(
            template,
            '<!-- 背景情報 -->',
            '<!-- メモ -->',
        ...
        self.assertIn('<label for="traits_mannerisms" class="form-label">メモ</label>', template)
        self.assertIn('name="secret_ho_info"', template)
        self.assertIn('私的メモ', template)
        self.assertIn('update-background-data', template)
        self.assertIn('delete data.edition;', template)
        ...
        for legacy_text in [
            ...
            self.assertNotIn(legacy_text, template)
    ...
    def test_legacy_view_skill_lists_are_split_by_edition(self):
        for relative_path in ['accounts/views/character_views.py', 'accounts/views_original.py']:
            with self.subTest(relative_path=relative_path):
