...
class Character6thTemplateRenderingTest(TestCase):
    ...
    def test_character_list_page(self):
        ...
        for i in range(3):
            create_test_character(self.user, name=f"Character {i+1}", age=25 + i)
    ...
    def test_character_edit_page(self):
        ...
        character = create_test_character(self.user, name="Edit Test", age=25)

        response = self.client.get(f"{reverse('character_create_6th')}?id={character.id}")
        self.assertEqual(response.status_code, 200)
    ...
    def test_unauthorized_access(self):
        ...
        character = create_test_character(other_user, name="Other's Character", age=25)
...
class Character6thFormValidationTest(TestCase):
    ...
    def test_invalid_ability_values(self):
        ...
        self.assertFalse(CharacterSheet.objects.filter(name="Invalid Character").exists())
    ...
    def test_missing_required_fields(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(CharacterSheet.objects.filter(user=self.user, age=25).exists())

    def test_character_update(self):
        """Test updating existing character"""
        character = create_test_character(self.user, name="Update Test", age=25)
...
class Character6thJavaScriptTest(StaticLiveServerTestCase):
    ...
    def login(self):
        ...
        WebDriverWait(self.selenium, 10).until(EC.url_contains("/accounts/dashboard/"))
    ...
    def activate_tab(self, tab_id, pane_id):
        tab = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, tab_id)))
    ...
    def test_ability_calculations(self):
        ...
        for ability, value in abilities.items():
            input_elem = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, ability)))
            self.selenium.execute_script("arguments[0].scrollIntoView({block: 'center'});", input_elem)
    ...
    def test_occupation_points_calculation(self):
        ...
        self.open_character_create_6th()
        self.activate_tab("abilities-tab", "abilities")
        WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "edu")))
        ...
        skills_tab = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "skills-tab")))
        self.selenium.execute_script("arguments[0].scrollIntoView({block: 'center'});", skills_tab)
        self.activate_tab("skills-tab", "skills")
        WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "occupationMethod")))
    ...
    def test_skill_allocation_interface(self):
        ...
        self.open_character_create_6th()
        skill_tab = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "skills-tab")))
        self.selenium.execute_script("arguments[0].scrollIntoView({block: 'center'});", skill_tab)
        self.activate_tab("skills-tab", "skills")

        WebDriverWait(self.selenium, 10).until(EC.visibility_of_element_located((By.ID, "skillsContainer")))
    ...
    def test_dice_roll_interface(self):
        ...
        self.open_character_create_6th()
        self.activate_tab("abilities-tab", "abilities")
        roll_all_btn = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "rollAllAbilities")))

        # Test roll-all functionality
        self.selenium.execute_script("arguments[0].scrollIntoView({block: 'center'});", roll_all_btn)
        roll_all_btn.click()
    ...
    def test_responsive_layout(self):
        ...
        try:
            main_tabs = WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "mainTabs")))
...
class Character6thAccessibilityTest(TestCase):
    ...
    def test_screen_reader_support(self):
        ...
        character = create_test_character(self.user, name="Screen Reader Test", age=25)
