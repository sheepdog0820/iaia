...
if SELENIUM_AVAILABLE:
    ...
    class JavaScriptErrorDetectionTest(StaticLiveServerTestCase):
        ...
        def open_main_tab(self, tab_id, pane_id):
            tab = WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, tab_id)))
            self.selenium.execute_script("arguments[0].scrollIntoView({block: 'center'});", tab)
            ...
            try:
                WebDriverWait(self.selenium, 10).until(EC.visibility_of_element_located((By.ID, pane_id)))
        ...
        def get_browser_errors(self):
            ...
            for entry in self.selenium.get_log("browser"):
                if entry["level"] == "SEVERE":
                    ...
                    if "favicon.ico" not in entry["message"] and "Failed to load resource" not in entry["message"]:
                        errors.append(entry)
            return errors
        ...
        def test_page_load_no_errors(self):
            ...
            WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "character-name")))

            # Check for JavaScript errors
            errors = self.get_browser_errors()
            self.assertEqual(len(errors), 0, f"JavaScript errors found on page load: {errors}")
        ...
        def test_ability_score_changes_no_errors(self):
            ...
            WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "dex")))
            ...
            # Check for JavaScript errors
            errors = self.get_browser_errors()
            self.assertEqual(len(errors), 0, f"JavaScript errors found when changing abilities: {errors}")
        ...
        def test_dice_rolling_no_errors(self):
            ...
            # Check for JavaScript errors
            errors = self.get_browser_errors()
            self.assertEqual(len(errors), 0, f"JavaScript errors found during dice rolling: {errors}")
        ...
        def test_skill_allocation_no_errors(self):
            ...
            # Check for JavaScript errors
            errors = self.get_browser_errors()
            self.assertEqual(len(errors), 0, f"JavaScript errors found during skill allocation: {errors}")
        ...
        def test_custom_base_value_editing_no_errors(self):
            ...
            WebDriverWait(self.selenium, 10).until(EC.visibility_of_element_located((By.CSS_SELECTOR, '[id^="base_"]')))
            ...
            # Check for JavaScript errors
            errors = self.get_browser_errors()
            self.assertEqual(len(errors), 0, f"JavaScript errors found during base value editing: {errors}")
        ...
        def test_form_submission_no_errors(self):
            ...
            # Check for JavaScript errors
            errors = self.get_browser_errors()
            self.assertEqual(len(errors), 0, f"JavaScript errors found during form submission: {errors}")
        ...
        def test_tab_navigation_no_errors(self):
            ...
            self.open_main_tab("skills-tab", "skills")
            WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "skillTabs")))
            ...
            # Check for JavaScript errors
            errors = self.get_browser_errors()
            self.assertEqual(len(errors), 0, f"JavaScript errors found during tab navigation: {errors}")
        ...
        def test_occupation_template_selection_no_errors(self):
            ...
            WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "occupationTemplateBtn")))
            self.selenium.execute_script("""
            ...
            # Check for JavaScript errors
            errors = self.get_browser_errors()
            self.assertEqual(len(errors), 0, f"JavaScript errors found during template selection: {errors}")
