...
class CharacterCreateAdvancedUITest(TestCase):
    ...
    def test_create_character_with_multiple_skills(self):
        ...
        response = self.client.post(
            "/api/accounts/character-sheets/", data=json.dumps(data), content_type="application/json"
        )
    ...
    class CharacterCreateAdvancedSeleniumTest(StaticLiveServerTestCase):
        ...
        def login(self):
            ...
            if "/accounts/login/" in (self.selenium.current_url or ""):
                ...
                WebDriverWait(self.selenium, 20).until(EC.url_contains("/accounts/dashboard/"))

        def open_main_tab(self, pane_id: str):
            """Open a main (top-level) tab in the character create UI."""
            tab = WebDriverWait(self.selenium, 20).until(EC.presence_of_element_located((By.ID, f"{pane_id}-tab")))
            self.selenium.execute_script("arguments[0].click();", tab)
            WebDriverWait(self.selenium, 20).until(EC.visibility_of_element_located((By.ID, pane_id)))
        ...
        def test_dex_to_dodge_skill_synchronization(self):
            ...
            dex_field = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "dex")))
            ...
            dodge_base = WebDriverWait(self.selenium, 10).until(EC.presence_of_element_located((By.ID, "base_dodge")))
            WebDriverWait(self.selenium, 10).until(lambda _driver: dodge_base.get_attribute("value") == "20")
            ...
            WebDriverWait(self.selenium, 10).until(lambda _driver: dodge_base.get_attribute("value") == "36")
        ...
        def test_occupation_template_with_skills(self):
            ...
            self.open_main_tab("abilities")
            edu_field = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "edu")))
            edu_field.clear()
            ...
            WebDriverWait(self.selenium, 10).until(lambda _driver: occupation_total.get_attribute("value") == "320")
        ...
        def test_multiple_skill_allocation(self):
            ...
            self.open_main_tab("abilities")
            edu_field = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "edu")))
            int_field = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "int")))
            edu_field.clear()
            ...
            for skill_id, points in skills_to_allocate:
                skill_input = self.selenium.find_element(By.ID, f"occ_{skill_id}")
                skill_input.clear()
            ...
            interest_skills = [("dodge", 20), ("handgun", 15), ("first_aid", 10)]

            for skill_id, points in interest_skills:
                skill_input = self.selenium.find_element(By.ID, f"int_{skill_id}")
                skill_input.clear()
                skill_input.send_keys(str(points))
                skill_input.send_keys(Keys.TAB)
                time.sleep(0.2)
        ...
        def test_allocated_skills_tab(self):
            ...
            allocated_names = [
                el.text.strip() for el in self.selenium.find_elements(By.CSS_SELECTOR, "#allocatedSkills .fw-bold")
            ]
        ...
        def test_complete_character_creation_flow(self):
            ...
            WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, "character-name")))
            ...
            for ability_id, value in {
                ...
                field = WebDriverWait(self.selenium, 10).until(EC.element_to_be_clickable((By.ID, ability_id)))
                field.clear()
            ...
            self.open_main_tab("skills")
            self.open_skill_tab("all")
            skills = [("spot_hidden", 40), ("psychology", 30), ("library_use", 20)]
            ...
            # Wait for redirect
            WebDriverWait(self.selenium, 10).until(EC.url_contains("/accounts/character/list/"))

            # Verify character was created
            self.assertTrue(CharacterSheet.objects.filter(name="完全なる探索者").exists())
