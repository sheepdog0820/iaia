...
候補ソースは `77b10ded69f467dea8f7a924dfc3f65faaa0da44`。配備用イメージのソースb6e58ebdとの差分は検証文書のみ。d572a618以降にBootstrap同梱・一覧取得改善が入っている。以下の過去候補の節は履歴として保持する。この資料は配備承認ではない。

| 項目 | 最新の証拠と限界 |
| --- | --- |
| 配備用イメージ | `tableno-formal-release:b6e58ebd`、ローカルID `sha256:8cd6229e7779e5abd5d2383379eae685909481f1a48ca27046e0b1b2de72408e`。通常Dockerfile/固定依存から作成。ECRへ未送信でmanifest digestは未取得 |
| 隔離起動 | 専用PostgreSQL16/Redis7で全移行・静的ファイル183件収集・Daphne起動・通常利用者の一覧200を確認。隔離HTTP用にSSL redirectを無効にしたcheck --deployはW008を1件報告。架空設定・使い捨てデータでの確認であり、実TLS/Stripe/OAuth/S3や実データ移行の証明ではない |
| 全体検証 | 77b10dedの別テスト用イメージでSQLite/PostgreSQL実行中。終了結果とリモートCIは未取得。過去候補の成功で代用しない |
