...
| accounts 0055 | 既存移行の修正。PGの遅延制約をインデックス作成前に検証する。12:44の検査で適用済み、通常のmigrateでは再実行されない |
| accounts 0058 | 既存移行の修正。旧登録列の削除をDB別に処理する。12:44に適用済み・旧登録列なしを確認。別DBで未適用ならデータ移送と列削除の復旧条件が必要 |
| schedules 0041 | 既存移行の修正。旧テンプレート画像削除の失敗を秘匿したログで報告。12:44の検査で適用済み。別DBで未適用なら従来のファイル削除処理も動くため、削除承認なしに実行しない |

[共有DBの読み取り検査](DATABASE_PREFLIGHT_2026-09-06.md)は本日12:44に承認済み範囲で完了していた。夕方にCloudWatchのJSONと保存結果の全項目一致を確認。accounts/0064とschedules/0055は未適用、既存accounts/0055・0058とschedules/0041は適用済みであり、通常の再実行対象ではない。検査コマンドは0b0cea6fから4d7c4ea7まで差分なし。前版の本節で未実施とした記載を訂正する。反映直前にはこの時点からの変更を確認するが、未実施を理由に同じ一時検査を繰り返さない。移行履歴のfake変更、既存移行の強制再実行、複数ロールの削除による逆移行回避は行わない。
...
1. 最新候補のECR manifest digest、既存Secretsによる起動要件、完了済み読み取りDB検査からの変更有無を確認する。追加検査が必要になった場合だけ対象を明示し、一時検査タスクはentrypointの移行・静的収集・開発ユーザー作成を起動しない構成に限定する。
...
## 最新候補のローカル準備（10d21e42、2026-09-08）

[固定候補の記録](RUNTIME_CANDIDATE_10D21E42_2026-09-08.md)に新しいイメージID・ソース照合・本番設定での隔離起動を記載した。ECR未送信、共有環境未反映。以下の01a52f52以前は履歴として保持し、最新候補の成功証拠には使わない。

共有DBの読み取り検査は[2026-09-08に実行済み](DATABASE_PREFLIGHT_2026-09-06.md)。accounts/0064とschedules/0055は未適用であり、下記旧記録の「共有DB検査未実施」は解消したが、DB変更・配備の承認を得た意味ではない。最終CI、OS監査の残指摘、実環境の復旧・費用・外部宛先を揃えてから具体的な配備承認を求める。
