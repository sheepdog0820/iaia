...
| 区分 | 現在の状態 |
| --- | --- |
| 開発環境 | Web定義45・コード20803480。フォント、背景透過の起動権限、セッション性能、統計表示の修正を反映済み |
| main / 作業候補 | mainは14ac4746。後続修正はDraft PR #3。新規mainマージ・本番反映は未実施 |
| 自動検査 | 稼働アプリのCIは成功。Terraform検査追加後のb37a9dd2は5ジョブ成功・単体/統合失敗。CIのジョブ数を5に固定する既存テストを31b032c8で修正し関連22件成功、全体CIは再検査中 |
...
| Q03 | リリース候補SHAのCI・固定依存関係・本番DB検証が成功する | 稼働アプリ20803480はpush/PR CI全5ジョブ成功。後続でTerraform検査を加え、b37a9dd2は5ジョブ成功・単体/統合失敗。ジョブ数の期待値を31b032c8で修正し関連22テスト成功、全6ジョブの再検査中。[Draft PR #3](https://github.com/sheepdog0820/iaia/pull/3)はopen、main向け。mainは14ac4746。accounts/0064・schedules/0055は適用終了0、残移行なしの記録あり | 最新候補の全CI完了と反映直前の差分を確認。全スキーマ/実データ監査、本番移行と復旧は未完了。新規mainマージは未承認 / Codex、人間 |
