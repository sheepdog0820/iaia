...
| B04 | Webhook重複・順序変動・一時DB障害で課金権限を失わず再処理できる | 実署名通知の重複・失敗再送・購読通知の逆順を確認。[重複購入防止・応答喪失の実API再送・PG同時購入](STRIPE_CHECKOUT_RETRY_2026-09-13.md)も成功 | [請求成功/失敗の逆順・別請求同時更新](STRIPE_INVOICE_ORDERING_2026-09-19.md)は修正し実署名通知・隔離PGで検証済み。複数返金/異議・他種Webhookの競合と変更後CIを確認 / Codex |
