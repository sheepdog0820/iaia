...
最新の反映状況: [d875d028のmainマージ・開発AWS反映](AWS_PRE_D875D028_2026-09-13.md)が完了。タスク定義49・稼働1・DB/キャッシュ正常、請求開示表記とCheckout停止をChromeで確認した。対象コードのpush/PR CI全6ジョブ成功。正式公開は引き続きNo-Goで、Stripe実検証・Google実同期・開示窓口の運用・性能/復旧等の残条件は継続する。作業中の添付操作画面は含まない。

以下の9月12日以前の候補・未反映・CI待ちの記載は当時の検証記録。現在の稼働版と反映結果は上記の最新記録を優先する。
