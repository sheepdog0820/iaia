...
最新のOS依存削減（2026-09-08）: [通常Dockerfileから不要なcurlを除去](RUNTIME_CURL_REMOVAL_2026-09-08.md)。新イメージはOS指摘41件（HIGH 1 / MEDIUM 1 / LOW 33 / UNSPECIFIED 6）で、従来の48件からLOW 7件を削減。通常起動・画像HTTP14件・ソース553ファイル一致を検証した。ベースc8a5de97のPR/push CIはsuccessだが、この変更後のCI・共有環境反映は未完了。下段の48件は旧候補の記録。Stripe保留・RDS復元承認待ち・正式公開No-Goは維持する。
...
| Q02 | 合意した人数・データ量・同時利用で応答/エラー率の目標を満たす | [一覧APIの4構成比較](READ_API_CAPACITY_2026-09-08.md)はbca01195で720要求すべて200。小CPU構成の同時利用で遅延を観測。[最新通常イメージの透過処理](BACKGROUND_REMOVAL_RUNTIME_2026-09-08.md)は単純図形の1,600万画素で62.50秒、プロセス最大RSS約1.39 GiB。いずれも単発のローカル試験で、合意した規模・閾値と実環境の合格証拠は未確定 | 合意した規模・基準で反復・長時間・本番相当試験（一覧以外の主要フローも含む） / Codex、合否基準は人間 |
...
- GitHub: [Draft PR #2](https://github.com/sheepdog0820/iaia/pull/2)は作成済み。2026-09-08にopen/draft、対象main、head 46530ebcをAPIで確認。通常push可能。PR作成用の接続を再度依頼する必要はない。CI完了とmainへのマージ承認は別の条件。
