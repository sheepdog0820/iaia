...
## 現在の公開判断（Stripe候補の更新: 2026-09-19）

Stripe候補 `69cfa876` の[公開条件照合](STRIPE_CANDIDATE_AUDIT_2026-09-19.md)を更新した。対象SHAのCI全6ジョブ成功を確認。9月13日の実サンドボックス試験により、以下の古い「登録保留」「実課金ライフサイクル未検証」は解消した範囲がある。B01〜B05の判定表を参照する。AWSの現在状態と課金候補の反映は今回確認しておらず、他分野の過去記録を最新の実証として扱わない。
...
| 自動検査 | [a1a50b63の通常配布物とCI](RUNTIME_CANDIDATE_A1A50B63_2026-09-12.md)：関連55テスト成功・506ファイル一致。push/PR全6ジョブ成功、単体・統合各1,828成功/30スキップ、PG293成功/48サブテスト、ブラウザ各186成功。候補のmain/AWS反映は未実施 |
...
| B03 | 更新・期間末解約・支払失敗/回復・返金/異議・退会で権限が過不足なく変化する | 月額・年額更新、失敗/回復、返金、異議、カード変更、期間末解約を実サンドボックスで確認。[解約後勝訴による誤付与も修正・実証](STRIPE_ADDITIONAL_VERIFICATION_2026-09-13.md) | [権限停止中・未終了契約の退会防止](STRIPE_DELETION_GUARD_2026-09-19.md)を修正し隔離DBで検証。[購入画面失効・Stripe残契約照会・退会中の購入防止](STRIPE_DELETION_CHECKOUT_2026-09-19.md)も修正しPG・実APIで検証。[退会画面の3ブラウザー検証](STRIPE_DELETION_BROWSER_2026-09-19.md)も成功。[退会後の実署名付き購読通知の逆順・重複](STRIPE_DELETION_SIGNED_2026-09-19.md)も13項目成功。実Stripe契約を伴うブラウザー退会、実署名通知との並行処理、複数返金/異議、AWS上の全フローを確認 / Codex |
| B04 | Webhook重複・順序変動・一時DB障害で課金権限を失わず再処理できる | 実署名通知の重複・失敗再送・購読通知の逆順を確認。[重複購入防止・応答喪失の実API再送・PG同時購入](STRIPE_CHECKOUT_RETRY_2026-09-13.md)も成功 | [請求成功/失敗の逆順・別請求同時更新](STRIPE_INVOICE_ORDERING_2026-09-19.md)は修正し実署名通知・隔離PGで検証済み。[複数停止理由の保持](STRIPE_MULTIPLE_REVOCATIONS_2026-09-19.md)も修正・実検証済み。[同一異議の通知逆順](STRIPE_DISPUTE_ORDERING_2026-09-19.md)も実署名通知・隔離PGで検証済み。[古い購入完了通知・同時処理](STRIPE_CHECKOUT_ORDERING_2026-09-19.md)も修正し単体/PG・実API読み取りを確認。[課金メールの永続キュー・再試行](BILLING_EMAIL_DELIVERY_2026-09-19.md)を実装し隔離PG・ローカルSMTPで検証。[候補99acd8caの全6ジョブCI・コンテナ/DB互換性](STRIPE_CANDIDATE_CONTAINER_2026-09-19.md)は成功。[実署名付きCheckout完了を解約後に処理する試験](STRIPE_CHECKOUT_SIGNED_ORDER_2026-09-19.md)も10項目成功。AWS配送、ログイン状態での購入戻り画面、その他Webhook競合を確認 / Codex |
...
| Q03 | リリース候補SHAのCI・固定依存関係・本番DB検証が成功する | [a1a50b63](RUNTIME_CANDIDATE_A1A50B63_2026-09-12.md)のpush/PR CI全6ジョブ成功、通常配布物55テスト成功。稼働中はf97c7809/開発AWS定義48で、後続候補は未反映 | 候補の反映・実環境確認、全スキーマ/実データ監査、本番相当の移行・復旧と正式公開は未完了 / Codex、人間 |
