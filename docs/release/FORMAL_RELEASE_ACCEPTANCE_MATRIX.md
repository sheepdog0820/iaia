...
| 区分 | 現在の状態 |
| --- | --- |
| 開発環境 | [Web定義46・コードf16eb72b](AWS_PRE_SHEETS_SELECTION_2026-09-10.md)。フォント、背景透過の起動権限、セッション性能、統計表示、Sheets空指定拒否を反映済み |
| main / 作業候補 | mainは14ac4746。後続修正はDraft PR #3。新規mainマージ・本番反映は未実施 |
| 自動検査 | 稼働コードf16eb72bのpush/PR CIは全6ジョブ成功。Terraform検査追加に伴う既存テストのジョブ数固定による失敗も解消。後続文書コミットのCIは別に確認する |
...
| Q03 | リリース候補SHAのCI・固定依存関係・本番DB検証が成功する | 稼働コードf16eb72bはpush/PR CI全6ジョブ成功。Terraform検査追加に伴う既存テスト失敗も修正・再検査済み。[Draft PR #3](https://github.com/sheepdog0820/iaia/pull/3)はopen、main向け。mainは14ac4746。accounts/0064・schedules/0055は適用終了0、残移行なしの記録あり | 後続候補のCIと反映直前の差分を確認。全スキーマ/実データ監査、本番移行と復旧は未完了。新規mainマージは未承認 / Codex、人間 |
