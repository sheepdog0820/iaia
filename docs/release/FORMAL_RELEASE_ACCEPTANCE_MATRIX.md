...
現況の照合（2026-09-08、作業ブランチ5b8544fb時点）: codex/formal-release-resumeへpush済み。同SHAの[CI](https://github.com/sheepdog0820/iaia/actions/runs/34024850828)は全5ジョブ成功。今回の作業ではmainへの追加マージ・実環境への配備は未実施。共通CSSの自動スクロール、テストの入力準備・データ分離、DockerコンテキストのTerraform作業ファイル除外、OSビルド用パッケージ除去まで反映した。下表Q01/Q03/Q04を更新し、過去候補の結果は履歴として残す。CIの本番DB検証は隔離PostgreSQLであり、共有環境のDB検査を完了した意味ではない。
