...
| Q03 | リリース候補SHAのCI・固定依存関係・本番DB検証が成功する | 7fc6b61eのPR/push CI全5ジョブ成功を確認。最新アプリ4d7c4ea7もPR/push全5ジョブ成功。通常イメージの隔離起動と画像・非公開ファイルの実HTTP34件を確認済み。[Draft PR #2](https://github.com/sheepdog0820/iaia/pull/2)はopen、main向けに作成済み。共有DBの履歴/制約検査は未実行 | DB検査案・配備案の旧SHAを対象候補へ更新し、共有環境検証と追加マージの承認範囲を確定 / Codex、人間 |
