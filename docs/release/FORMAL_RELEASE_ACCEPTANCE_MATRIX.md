...
| 区分 | 現在の状態 |
| --- | --- |
| 開発環境 | [Web定義47・コード595a390a](AWS_PRE_GOOGLE_RETRY_2026-09-10.md)。フォント、背景透過の起動権限、セッション性能、統計表示、Sheets空指定拒否、再試行の権限受付と対象維持を反映済み |
| main / 作業候補 | mainは14ac4746。後続修正はDraft PR #3。新規mainマージ・本番反映は未実施 |
| 自動検査 | 稼働コード595a390aのpush/PR CIは全6ジョブ成功。通常配布物の関連27テスト・実HTTP19ケースも成功。後続文書コミットのCIは別に確認する |
...
| Q03 | リリース候補SHAのCI・固定依存関係・本番DB検証が成功する | 稼働コード595a390aはpush/PR CI全6ジョブ成功。Terraform検査追加に伴う既存テスト失敗も修正・再検査済み。[Draft PR #3](https://github.com/sheepdog0820/iaia/pull/3)はopen、main向け。mainは14ac4746。accounts/0064・schedules/0055は適用終了0、残移行なしの記録あり | 後続候補のCIと反映直前の差分を確認。全スキーマ/実データ監査、本番移行と復旧は未完了。新規mainマージは未承認 / Codex、人間 |
| Q04 | 情報保護の重大な既知欠陥がなく、セキュリティ指摘に判定がある | 非公開ファイル保護を反映し、実S3/CDNの12パターン拒否と認可付き取得を実証済み。過去のOSスキャンは41件（HIGH1/MEDIUM1/LOW33/UNSPECIFIED6）で、判定・解消は未完了 | 最新配布物のOS再確認、指摘の適用条件/残リスク判定、全配送・権限経路監査を継続 / Codex |
| O01 | 本番設定・秘密情報・監視・問い合わせ配送が正しく構成され実証される | Web定義45が稼働。過去の監視確認ではアラーム3件・SNS購読confirmed、実通知到達は未確認。現時点でworker/beatサービスとブローカーなし。ネットワーク・DBセッション維持設定はローカル修正済み | 一時AWS通信試験は成功・削除済み。継続運用/夜間停止・監視・問い合わせ配送を別途実証 / Codex、人間承認 |
