...
- 固定ソース704c6f06のChromium/Firefox/WebKitは33件成功（4.5分、retryなし）。通常登録、グループ権限変更、通知既読/招待承認、保存文字列の安全な表示、セッション作成/完了/非参加者拒否、日程調整、CDN停止時の動作に加え、ゲスト招待発行・未ログイン参加・登録後の引き継ぎと入力エラーからの再試行を画面で確認。全E2Eファイルを網羅した結果ではない。
