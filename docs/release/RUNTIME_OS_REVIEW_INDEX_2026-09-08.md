...
最新の再ビルド（2026-09-08）: [不要なcurlを除去](RUNTIME_CURL_REMOVAL_2026-09-08.md)した通常イメージ `158c7301a238` は41指摘（HIGH 1 / MEDIUM 1 / LOW 33 / UNSPECIFIED 6）、終了2。curlのLOW 6件とGnuTLSのLOW 1件が搭載パッケージの削減でなくなった。以下の48件の一覧は変更前の監査記録であり、最新件数と区別する。残る条件と正式公開No-Goは維持する。
...
2. 新しい通常イメージではlibcurl → librtmp → GnuTLS経路を削除した。今後依存を追加する際は、この経路の再導入とTLS設定を再評価する。外部HTTPS・DB TLSの実環境検証は別途必要。
