...
1b85f6f2後、同じ本番用イメージと修正済み推論関数で、空の使い捨てSQLiteを移行し、非管理者の有料ユーザーと別ユーザーを作成した。APIClientによるジョブ作成は202、処理前の状態取得も202だった。AWSタスク起動関数だけを模擬にし、`process_background_removal_job`管理コマンドは本物のU²-Net推論・ファイル保存・DB更新を実行した。

1 CPU・2 GiB・スワップなしで完了し、ジョブはcompleted、元画像のフィールドと実ファイルが削除された。所有者の結果取得200、PNG/RGBA・256×384・alpha最小0/最大255、別ユーザーの取得404を確認。有料権限を失効させても保存済み結果の内容は一致し、新規処理は403・ジョブ数1のままだった。登録から検証完了まで68.54秒、終了0。証跡tmp/background-job-probe.log、tmp/background-job-proof/result.json・result.png。ログの404/403は期待した拒否結果である。

この検証は実Fargate起動、実S3、実Stripeによる失効、ブラウザ、保持期間後の削除を含まない。コンテナと使い捨てDBは終了時に破棄された。

同日の読み取り確認でaws-preの背景透過タスク定義はtableno-aws-pre-background-removal:2、ACTIVE、CPU1024・メモリ2048、イメージタグaws-pre-2b2f02a3だった。今回の修正済みイメージが配備された証拠ではなく、タスク起動・定義更新・権限変更はしていない。
