...
後続更新: [承認対象と完成した操作案](RDS_RESTORE_APPROVAL_2026-09-08.md)に最小タスク定義・SDK検証・後片付けと追加費用を整理した。既存ECRイメージの再利用は成立せず、検証済みイメージを専用タグへ一時送信する案へ変更。税・予備費込み目安約0.60 USD、予算案1 USD。以下の「既存イメージ再利用」「残費用未算入」は初期案の履歴であり、最新案はリンク先を参照する。
...
- [probeの実コードと検証](RDS_RESTORE_PROBE_2026-09-08.md)は追加済み。単体8件・新規行/分岐100%、合成PG18のTLS接続・書き込み拒否・秘匿ログを確認。実RDS接続は未実施。
