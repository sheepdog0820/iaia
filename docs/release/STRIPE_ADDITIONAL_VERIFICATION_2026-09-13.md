...
購読Webhookの順序逆転は下記の追加修正・実検証で確認した。請求・異議申し立てをまたぐ順序逆転、PostgreSQLでの並行実行、二重契約防止、年額Checkout画面と3D Secure、支払い方法更新画面、開発AWSの設定・Webhook・戻り導線、実運用メール・法務・販売開始条件は残る。この記録は正式公開の最終合格証拠ではない。
