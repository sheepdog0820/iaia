...
| 項目 | 値 |
| --- | --- |
| 確認日 | 2026-06-21 07:12:54 JST |
| 確認者 | 未記入 |
| 環境 | local |
| base URL | 未設定 |
| Stripe mode | test |
| Price ID | 未設定 |
| Webhook endpoint URL | /api/billing/webhook/ |
| リリース/commit | 未記入 |
...
| Billing status report | `python manage.py billing_status_report` | OK | last_webhook_event_id=(none), last_webhook_event_type=(none), last_webhook_processed_at=(none), failed_webhook_events=0, stale_processing_webhook_events=0, last_failed_webhook_event_id=(none), last_stale_processing_webhook_event_id=(none) |
| Stripe remote check | `python manage.py billing_stripe_remote_check` | 未実行 | 必要に応じて--run-remote-checkを指定 |
| ローカル状態遷移スモーク | `python manage.py billing_webhook_smoke` | OK | billing_webhook_smoke=ok user=billing-smoke-user audits=27 |
...
| Premium subscriptions | Stripe ID、ステータス、期限、失効理由、返金/チャージバック後の確認状態が見える | 要確認 |  |
...
| Premium audit logs | 付与、復旧、失効、支払い失敗、支払い復旧、返金、チャージバック、管理者操作actorが残る | 要確認 |  |
