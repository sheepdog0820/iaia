...
`billing_verification_record` writes the checked timestamp, environment, monthly Price ID, yearly Price ID, Webhook URL, `check`, `billing_preflight`, optional `billing_stripe_remote_check`, optional `billing_webhook_smoke`, and manual Stripe/Django admin verification items to Markdown. Add real Stripe event IDs and screen verification notes to the generated file.
...
```bash
python manage.py issue_premium_code --label campaign-2026 --campaign spring-2026 --max-uses 100 --expires-in-days 30
```
...
Celery beatが有効な環境では `schedules.tasks.expire_premium_access` が1時間ごとに実行されます。
...
```bash
python manage.py set_premium_user username --on --reason "support comp until migration completes"
python manage.py set_premium_user username --off --reason "support comp ended"
```

`billing_status_report` はステータス別件数、現在有効なプレミアム権限数、手動付与中ユーザー数、課金レコードを持たない手動付与ユーザー数、解約予定、支払い失敗、返金/チャージバック、返金/チャージバック検知後も有効な権限、期限切れ運営コード、無期限コード由来プレミアム、期限付き有効コード由来プレミアム、7日以内に期限切れになるコード由来プレミアム、キャンペーン別のコード由来プレミアム内訳、ユーザー権限ずれ、最後に処理したStripe Webhook event ID/種別/時刻、失敗または処理中のWebhook event数を出力します。`active_access` には課金レコード由来の有効権限に加え、課金レコードを持たない手動付与ユーザーも含まれます。15分を超えて `processing` のまま残るWebhook eventは `stale_processing_webhook_events` として別に集計します。`--fail-on-issues` は権限ずれまたは期限切れpromoがある場合にエラー終了します。`--include-payment-issues` を併用すると、支払い失敗、返金/チャージバック検知、失敗Webhook event、古い処理中Webhook eventもエラー条件に含めます。`manual_override_users` が増えている場合は、運営判断の手動付与ユーザーとして監査ログの理由と期限管理方針を確認します。`manual_override_without_subscription` が増えている場合は、課金レコードなしの手動付与ユーザーとして、ユーザー詳細の監査ログまたは `set_premium_user --off` の解除予定を確認します。`promo_expiring_soon_records` が増えている場合は、期限付きコード由来プレミアムが7日以内に失効するユーザーとして課金画面と管理画面の表示を確認します。`promo_campaigns` ではキャンペーンごとの total / active / expired / expiring_soon / indefinite を確認します。`cancel_at_period_end_records` が増えている場合は、次回更新日まで権限が有効な解約予定ユーザーとして課金画面と管理画面の表示を確認します。`refund_or_dispute_active_records` が0でない場合は、`STRIPE_REVOKE_ON_REFUND_OR_DISPUTE=False` の手動確認運用で対応待ちの可能性があります。`failed_webhook_events` が0でない場合は、Django管理画面の `Stripe webhook events` で `processing_status=failed` と `error_message` を確認し、Stripe Dashboardから該当eventを再送します。`stale_processing_webhook_events` が0でない場合は、アプリログとプロセス停止の有無を確認し、Stripe Dashboardから該当eventを再送して `succeeded` になることを確認します。`last_webhook_processed_at` が想定より古い場合は、Stripe DashboardのWebhook送信状況、アプリログ、`STRIPE_WEBHOOK_SECRET`、公開URLを確認します。`reconcile_premium_access` は期限切れ運営コード由来プレミアムを先に失効し、課金レコードを正としてユーザーのプレミアム権限を再同期します。変更時は `Premium audit logs` に記録されます。 JSON includes `billing_intervals` with `month`, `year`, and blank interval counts.
