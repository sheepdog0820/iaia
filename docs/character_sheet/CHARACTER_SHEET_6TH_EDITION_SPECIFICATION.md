...
#### 複数画像対応
- メイン画像設定
- 追加画像アップロード（無料・プレミアム共通で1キャラクター最大5枚。旧上限で保存済みの画像は保持）
- 画像の並び替え（ドラッグ&ドロップ対応）
- 画像の削除
- 元画像URLの提供
- 画像ギャラリー表示
...
ブラウザのキャラクター詳細・セッション画面からコピーする場合、SAN判定は出力済みのSANステータスを参照する `CCB<={SAN} 【SANチェック】` 形式とする。固定の現在SAN値はコマンドへ埋め込まない。
...
- **画像管理システムの強化**
  - 複数画像アップロード（無料・プレミアム共通で1キャラクター最大5枚）
  - メイン画像切り替え機能
