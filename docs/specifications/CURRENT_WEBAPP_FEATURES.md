...
- **内容**
  - 複数画像アップロード（無料・有料共通で1キャラクター最大5枚。既存の上限超過画像は維持）
  - メイン画像の切替
  - 画像の並び替え
...
- **内容**
  ...
  - PL一覧ヘッダーのボタンから、シナリオの推奨・準推奨技能を参加キャラクターごとにモーダルで比較表示
    - 保存済み技能は現在値、未配分の標準技能は初期値を表示
    - GM・共同GM・参加PL向けの内部表示とし、公開共有画面では非表示
  - グループの仮ユーザーが参加者に含まれる場合のみ、PL一覧ヘッダーのボタンから参加者紐付け申請モーダルを表示
  - 参加キャラクターカードの情報・操作領域をコンパクトに整理し、文字と操作ボタンを左揃えで表示
    - 「出力」の左に「参照」「編集」「削除」を配置し、参照先は内部キャラクターの直接URLまたは登録済み外部URLを新しいタブで開く
    - URLコピーは参照先と同じURLをコピーし、内部キャラクターのJSON出力は「出力」として提供
- **状態**: **完成**
...
- **内容**
  ...
  - メモの更新・削除は作成者のみ可能（他者の公開メモへの変更は403扱い）
- **API（代表）**
  - `/api/scenarios/notes/`
