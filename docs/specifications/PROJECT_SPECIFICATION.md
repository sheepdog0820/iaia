...
### 2026-07-02 Current share links and schedule import

本節は、下位節に残る古い `private/public` や `share_token` の説明より優先します。
詳細は `docs/specifications/SAFE_SHARE_LINKS.md` を参照してください。

- **公開範囲の意味**
  - `private`: 所有者または管理権限者のみ。
  - `group`: 所属グループ内の権限に従う。
  - `link`: visible only through fixed share URLs or issued `ShareLink` URLs; it is not listed publicly and is not readable by ID.
  - `public`: readable through shared URL/API surfaces and may also have issued `ShareLink` URLs.
- **ShareLink**
...
- **Schedule import**
  - `import_trpg_schedule` supports Excel parsing (`--excel-path`) and prepared JSON import (`--input-json`) only.
  - `--extract-only` writes parsed Excel output without importing to the database.
  - `--dry-run` does not write to the database and reports planned session, participant, and YouTube-link counts.
  - Direct legacy CSV import and legacy identifier columns were removed before public release.
...
- **管理者アカウント**: 
  - ユーザー名: admin
  - パスワード: `python scripts/dev/create_admin.py` 実行時に表示される値
- **サンプルデータ**:
  - ユーザー数: 12名
  - キャラクター数: 43体
  - セッション数: 11件
...
# スーパーユーザー作成
python3 scripts/dev/create_admin.py
...
- セッション詳細のPL一覧では、内部キャラクターが設定されている場合にヘッダーボタンから推奨・準推奨技能の比較モーダルを表示
  - 保存済み技能は現在値、未配分の標準技能は版・能力値に応じた初期値を表示
  - 一般名で指定された専門技能は、キャラクターが保持する該当専門と値をすべて表示
  - GM・共同GM・参加PLのみ閲覧でき、非参加者と公開共有画面には表示しない
  - グループの仮ユーザーが参加者に含まれる場合、PL一覧ヘッダーのボタンから参加者紐付け申請モーダルを表示する
    - 仮ユーザーが参加者に含まれない場合は、ボタンとモーダルを表示しない
  - PLカードは参加者名・キャラクター名・操作をコンパクトに整理し、文字と操作ボタンを左揃えで表示
    - 「出力」の左に「参照」「編集」「削除」を表示し、既存のキャラクター設定・除名機能を引き継ぐ
    - 参照とURLコピーは、内部キャラクターでは認証付き直接URL、外部キャラクターでは登録URLを使用
    - ココフォリア貼り付け用JSONのコピーは「出力」と表示し、JSON形式であることをツールチップで案内
...
- `GET /share/sessions/<uuid:share_token>/view/` - fixed session share page for sessions with `visibility='link'` or `visibility='public'`
...
- **セッション準備チェックリスト**【🚫 取り下げ】
  - 当面不要のため実装対象外
- **経験値配布**【✅ 実装済み】
  - 配布内容の記録（SessionReward）
  - GM/co-GMによる入力UI（セッション詳細）
  - キャラクター成長記録への反映（任意）
- **セッション後フィードバック**【🚫 取り下げ】
  - 当面不要のため実装対象外
- **セッションテンプレート**【✅ 実装済み】
...
- **グループ統計詳細**【✅ 実装済み】
  ...
  - 人気シナリオランキング
- **ユーザーランキング詳細**【✅ 実装済み】
  - 期間別ランキング（month/all/range）
  - カテゴリ別ランキング（system/scenario）
  - ランキング推移（month/year）
...
- `python scripts/dev/create_admin.py` - 管理者アカウント作成
