# タブレノ（Tableno）現状のWEBアプリ機能一覧（2026-01-18 時点）
...
- **内容**
  - 通知種別: ハンドアウト作成/公開/更新、セッション招待/変更/キャンセル/リマインダー、グループ招待、フレンドリクエスト/承認
  - 未読/既読管理、未読件数取得
  - 通知設定（種別ごとのON/OFF、メール/ブラウザ通知設定）
- **API（代表）**
  - `/api/schedules/notifications/`（一覧・既読化・未読数）
  - `/api/schedules/notification-preferences/`（通知設定）
- **状態**: **完成**（API/DB完成、通知一覧の専用UIは未整備）
...
以下は `ISSUES.md` 等で管理されている「今後の候補」で、現時点では未実装（または部分実装）の領域です。

- **ハンドアウト段階的公開**
  - 条件付き公開、時間指定公開
  - **状態**: **未完成**
- **セッション準備・事後処理ツール**
  - 準備チェックリスト、フィードバック、経験値配布、テンプレート
  - **状態**: **未完成**
- **外部連携**
  - Discord Webhook通知、カレンダー購読（ICSフィード）、Google Calendar同期、外部シート連携
  - ICSエクスポート（ファイルDL）は実装済み
  - **状態**: **一部完成**
- **セッション分析ダッシュボード**
  - 参加率、人気時間帯、GM負荷、プレイヤー相性分析
  - **状態**: **未完成**
- **グループ間連携機能**
  - グループ連携モデル、連携申請/承認フロー
  - **状態**: **未完成**
- **統計機能詳細**
  - グループ統計: メンバー参加率、人気シナリオランキング（基本統計は実装済み）
  - ユーザーランキング: 期間別、カテゴリ別、推移（基本ランキングは実装済み）
  - **状態**: **一部完成**
- **非同期処理（Celery）**
  - 重い統計処理/エクスポート処理の非同期化
  - **状態**: **未完成**
- **APIドキュメント自動生成**
  - drf-spectacular導入、OpenAPI/Swagger対応
  - **状態**: **未完成**
- **リアルタイム通知**
  - WebSocket によるリアルタイムPush通知
  - **状態**: **未完成**
- **モバイルアプリ**
  - iOS/Android対応、プッシュ通知、オフライン機能
  - **状態**: **未完成**
- **AI分析/推奨機能**
  - プレイスタイル分析、シナリオ推奨、セッションマッチング
  - **状態**: **未完成**
