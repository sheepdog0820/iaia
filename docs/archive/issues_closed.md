...
## 2026-08-23 権限・共有URLの正常化

### ✅ ISSUE-078: セッション作成者権限とPL兼任登録（解決済み）

- **解決日**: 2026年8月23日
- 作成者の `owner` と、卓内役割の `gm` / `player` を `SessionParticipantRole` で独立して保存する。
- 参加者とロールの組み合わせを一意にし、作成者のみ、GM、PL、GM+PLの各状態を許可する。
- `owner` だけでは秘匿HO、GMメモ、GM向けシナリオ情報を扱えない。
- 作成者がPLを兼任する場合は、自分のキャラクターシートを自身の参加者レコードへ紐づけられる。
- サービス、API、画面、マイグレーション、権限境界の回帰テストを更新した。

### ✅ ISSUE-079: セッション直接URLリンクの正規共有URL対応（解決済み）

- **解決日**: 2026年8月23日
- 正規共有URLを `/share/sessions/<uuid:share_token>/view/` に統一した。
- `private` / `group` セッションの共有URLを非公開とし、共有レスポンスから秘匿情報を除外するテストを整備した。
- 旧直接URLを廃止し、URL設定・テンプレート・仕様書の再混入を静的テストで検知する。
