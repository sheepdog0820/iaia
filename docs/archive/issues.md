...
- [x] Google Sheets出力を100行単位に分割し、チャンク成功ごとの進捗と途中失敗時の部分反映警告を追加した。205行の分割・進捗、201行の2回目失敗、不正A1範囲・応答の非露出をローカルmockで確認した。詳細は[検証記録](../release/GOOGLE_SHEETS_LARGE_EXPORT_2026-09-22.md)を参照する。

**未完了タスク**
- [x] Google Calendar API障害時に、失敗状態と再試行導線が残ることをローカルmockで確認する。
- [x] Google Sheets大規模エクスポートで進捗と失敗理由をローカルmockで確認する。
...
**2026-09-22 進捗**
- [x] Tindalos Metrics画面のAPI由来名称が未エスケープで動的HTMLへ挿入される問題を修正し、悪意ある値が文字として表示されイベントが実行されないことを3ブラウザで確認した。詳細は[検証記録](../release/STATISTICS_DYNAMIC_HTML_SECURITY_2026-09-22.md)を参照する。
- [x] 6版・7版キャラクター作成画面のカスタム技能名をHTMLへ直接挿入する経路を修正し、タグ・イベント属性を含む名前が文字として保持されることを3ブラウザ6件で確認した。詳細は[検証記録](../release/CHARACTER_CUSTOM_SKILL_SECURITY_2026-09-22.md)を参照する。
- [x] 同候補の初回CIで既存の通常利用者E2EがWebKitで再試行成功のflakeとなったため、ブラウザーCookieを使う保存確認と複合フローの時間枠へ修正した。ローカル6件と`1b051dba`のCI全6ジョブが成功した。
