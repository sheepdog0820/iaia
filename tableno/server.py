"""Daphne entry point that keeps bearer invitation URLs out of access logs."""
...
INVITATION_PATH = re.compile(r"(/(?:api/)?group-invitations/)[^/]+")
...
def private_action_logger(logger):
    def log(protocol, action, details):
        ...
        if "path" in safe_details:
            ...
            path = INVITATION_PATH.sub(r"\1[redacted]", path)
